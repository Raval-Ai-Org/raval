...
export function parseToolCalls(text: string): { calls: ChatToolCall[]; cleaned: string } {
  ...
  const cleaned = text.replace(TAG_RE, (raw, kind, attrs) => {
...
const CANVAS_ALIASES: Record<string, CanvasType> = {
  "social-post": "social-post", social: "social-post", linkedin: "social-post", instagram: "social-post", tweet: "social-post", x: "social-post",
  "seo-brief": "seo-brief", brief: "seo-brief", seo: "seo-brief",
  "landing-page": "landing-page", landing: "landing-page",
  email: "email", newsletter: "email",
  article: "article", blog: "article", post: "article",
  "design-asset": "design-asset", design: "design-asset", creative: "design-asset",
};
...
export async function executeToolCall(call: ChatToolCall, ctx: ExecuteCtx): Promise<ChatToolResult> {
  ...
  switch (call.kind) {
    ...
    case "open-studio": {
      ...
      if (brief) {
        ...
        try { sessionStorage.setItem(`studio:prefill:${canvas}`, brief); } catch { /* noop */ }
        window.dispatchEvent(new CustomEvent("studio:prefill", { detail: { canvas, brief } }));
      }
      return { kind: call.kind, ok: true, label: `Opening ${canvas.replace("-", " ")} studio`, detail: brief ? "brief prefilled" : undefined };
    }
    case "open-memory": {
    ...
    case "save-memory": {
      ...
      try { await ctx.saveMemory(title, body); }
      catch { return { kind: call.kind, ok: false, label: "Couldn't save to Memory" }; }
    ...
    case "schedule": {
      ...
      const channel = call.params.channel || (canvas === "email" ? "email" : canvas === "article" ? "blog" : canvas === "landing-page" ? "web" : "linkedin");
      const kind = canvas === "email" ? "email" : canvas === "article" ? "blog" : canvas === "landing-page" ? "landing" : canvas === "seo-brief" ? "brief" : "post";
      ...
      if (error) return { kind: call.kind, ok: false, label: `Couldn't schedule "${title}"`, detail: error.message };
      return { kind: call.kind, ok: true, label: when ? `Scheduled "${title}" for ${formatWhen(when)}` : `Saved draft "${title}"` };
    }
  }
