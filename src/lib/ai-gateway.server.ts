...
function trimMessages(messages: ChatMessage[], budgetChars = MAX_INPUT_CHARS): ChatMessage[] {
  ...
  return messages.map((m) => {
    ...
    if (m.content.length <= budget) { budget -= m.content.length; return m; }
...
function mapStatus(status: number, body: string): AiGatewayError {
  if (status === 401) return new AiGatewayError(401, "Image provider rejected the API key. Please update OPENROUTER_API_KEY.");
  if (status === 402) return new AiGatewayError(402, "AI credits exhausted on OpenRouter. Please top up at openrouter.ai/credits.");
  if (status === 429) return new AiGatewayError(429, "AI rate limit reached. Please try again in a moment.");
...
async function fetchWithTimeout(url: string, init: RequestInit, timeoutMs: number, timeoutMessage?: string): Promise<Response> {
  ...
  try {
    ...
    throw new AiGatewayError(502, `Network error contacting AI provider: ${String(error?.message ?? error).slice(0, 200)}`);
  } finally {
    clearTimeout(timer);
  }
}
...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const delayMs = Number.isFinite(retryAfter) && retryAfter > 0 && retryAfter <= 10
        ? retryAfter * 1000
...
export async function chatCompletion(
  ...
  const run = async () => {
    ...
    catch { throw new AiGatewayError(502, "AI provider returned malformed JSON"); }
    if (!json?.choices?.[0]?.message) {
      throw new AiGatewayError(502, "AI provider returned no completion");
    }
    const finish = json?.choices?.[0]?.finish_reason;
    if (finish === "length") {
      console.warn(`[ai-gateway] finish_reason=length (model=${body.model}, max_tokens=${capped}) — output truncated`);
    }
    if (cacheable && cacheKey) cacheSet(responseCache, cacheKey, json, opts.cacheTtlMs ?? DEFAULT_CACHE_TTL_MS);
    return json;
  };
...
export async function chatCompletionStream(opts: ChatOptions): Promise<Response> {
  ...
  const upstream = await fetchWithTimeout(`${OPENROUTER_BASE}/chat/completions`, {
    method: "POST",
    headers: headers(key),
    body: JSON.stringify(body),
  }, STREAM_TIMEOUT_MS, "AI provider did not respond in time. Please retry.");
...
export async function imageGenerationStream(opts: {
  ...
  if (!image) {
    ...
    image = await dedupe(`img:${cacheKey}`, async () => {
      ...
      }, { timeoutMs: IMAGE_TIMEOUT_MS, timeoutMessage: "Image provider timed out. Please retry — no image was returned." });
      ...
      let json: any;
      try { json = await res.json(); }
      catch { throw new AiGatewayError(502, "Image provider returned malformed JSON"); }
      ...
      if (!b64) {
        ...
        if (typeof url === "string" && /^https:\/\//i.test(url)) {
          const imgRes = await fetchWithTimeout(url, {}, IMAGE_URL_TIMEOUT_MS, "Image download timed out.");
          if (imgRes.ok) {
      ...
      if (!b64) {
        ...
        throw new AiGatewayError(502, providerMsg ? `Image provider: ${providerMsg}` : "Image provider returned no image data");
      }
      const payload: GeneratedImagePayload = { b64, mimeType: mimeType || detectImageMimeType(b64) };
