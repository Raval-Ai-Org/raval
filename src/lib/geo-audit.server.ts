// geo-audit.server.ts — the SEO / GEO / AEO audit engine behind
// /api/geo-audit: fetch a site's homepage, robots.txt, sitemap and llms.txt,
// then score AI-engine access, structured data, crawlability, content and
// trust signals. Deterministic — no model calls.
import "server-only";
...
function parseRobotsGroups(robots: string): RobotsGroup[] {
  ...
  for (const raw of robots.split(/\r?\n/)) {
    ...
    if (key === "user-agent") {
      // A User-agent after rules starts a new group; consecutive ones extend it.
      if (!current || sawRule) {
...
export function parseRobotsAllow(robots: string, bot: string): RobotsVerdict {
  ...
  if (specific.length) return verdictFor(specific);
  const wildcard = groups.filter((g) => g.agents.includes("*"));
  if (wildcard.length) return verdictFor(wildcard);
  return "allow";
}

// Bodies past this are truncated; the payload check still sees them as heavy.
const MAX_AUDIT_BYTES = 2 * 1024 * 1024;

function fetchAuditText(url: string): Promise<string> {
...
export async function runAudit(rawUrl: string) {
  const base = assertPublicUrl(normalizeUrl(rawUrl));
  const origin = `${base.protocol}//${base.host}`;

  const [homeHtml, robots, sitemap, llms, llmsFull] = await Promise.all([
    fetchAuditText(base.toString()),
    fetchAuditText(`${origin}/robots.txt`),
    fetchAuditText(`${origin}/sitemap.xml`),
    fetchAuditText(`${origin}/llms.txt`),
    fetchAuditText(`${origin}/llms-full.txt`),
  ]);
