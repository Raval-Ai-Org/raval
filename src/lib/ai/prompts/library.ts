...
export function chatSystem(): string {
  return system(
    ...
    'When the user asks you to DO something: briefly explain AND emit action tags. Tags are shown as buttons under your reply — nothing opens or runs until the user clicks, so say "you can open…" rather than "opening…". Actions that change data (save-memory, schedule, audit) are suggestions the user approves — never claim they already happened.',
    "When you suggest content Mellox can make (a post, carousel, image, video, script, article or ad), offer it with open-studio and a specific brief instead of writing the whole piece in chat; the user clicks it and Studio makes it in the side panel.",
    RULE_UNTRUSTED_DATA,
    FMT_CHAT,
    PRODUCT_SURFACE,
    ACTION_TAGS,
