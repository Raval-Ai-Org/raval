      ...
      items: {
        ...
        properties: {
          ...
          options: {
            ...
            items: {
              type: "object",
              properties: { value: { type: "string" }, label: { type: "string" }, hint: { type: "string" } },
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const concreteHint = /\b(instagram|linkedin|x|twitter|tiktok|youtube|facebook|blog|email|newsletter|reel|carousel|thread|caption|hook|cta|seo|meta|title tag|h1|schema|audit|competitor|keyword|calendar|schedule|generate|write|draft|rewrite|translate|summar|analy[sz]e|explain|list|outline)\b/i.test(p);
        const looksActionable = /^(write|draft|generate|create|make|build|plan|schedule|post|publish|analyze|analyse|audit|research|find|list|explain|summarize|summarise|rewrite|translate|outline|compare|score|check|fix)\b/i.test(p);
        ...
        try {
          const parsed = await runTool<Result>({
            ...
            description: "Ask 1-3 short multiple-choice questions to disambiguate the prompt. Skip if already concrete.",
