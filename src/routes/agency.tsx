...
import {
  ArrowRight, Sparkles, CheckCircle2, Clock, AlertTriangle,
  Calendar, TrendingUp, Users, Layers, ChevronRight, Lightbulb,
  Bell, Coffee, BarChart3, ListTodo, Inbox, Activity, Plus,
  Eye, Heart, MousePointerClick, Target, Check, X as XIcon, Loader2,
  CalendarClock, SkipForward, Filter, Command as CommandIcon, Copy,
  MessageCircle, Repeat2, Bookmark, Send, ThumbsUp, Globe, Search as SearchIcon,
  ImagePlus, Upload, Trash2, Zap, ImageIcon,
...
import {
  MOCK_APPROVALS, MOCK_SCHEDULED, MOCK_RECENT, TILE_BY_ID,
...
export const Route = createFileRoute("/agency")({
  component: AgencyHQ,
  head: () => pageHead({
    title: "Agency HQ · Raval AI",
    description: "Manage all clients at once. Approvals, schedules, activity and combined analytics — one Marketing Intelligence Layer across every brand.",
    path: "/agency",
    noindex: true,
    jsonLd: webPageLd({
      title: "Agency HQ · Raval AI",
...
const TINT: Record<string, string> = {
  "brand-blue": "#3b82f6", "brand-green": "#22c55e", amber: "#f59e0b",
  sky: "#0ea5e9", violet: "#8b5cf6", rose: "#f43f5e", teal: "#14b8a6", fuchsia: "#d946ef",
};
...
function AgencyHQ() {
  ...
  const [previewItem, setPreviewItem] = useState<null | {
    id: string; title: string; clientName: string; channel?: string; canvas: CanvasType;
  ...
  const refreshFromDb = async (ids: string[]) => {
    ...
    const [apr, ci] = await Promise.all([
      supabase.from("approvals")
        ...
        .in("workspace_id", ids).eq("status", "pending")
        .order("created_at", { ascending: false }).limit(50),
      supabase.from("content_items")
        .select("id, workspace_id, agent, kind, channel, title, body, hashtags, media_url, status, scheduled_at, created_at")
        .in("workspace_id", ids).order("created_at", { ascending: false }).limit(200),
    ]);
  ...
  useEffect(() => {
    ...
    const onLocal = () => { void refreshFromDb(ids); };
    ...
    (async () => {
      ...
      await Promise.all(need.map(async (c) => {
        try {
          ...
          let brandCtx = "";
          try {
            const raw = localStorage.getItem(`brand-dna:v3:${c.id}`)
          ...
          if (!brandCtx) brandCtx = `Brand: ${c.name}${c.website_url ? `\nWebsite: ${c.website_url}` : ""}`;
  ...
  const combinedApprovals = useMemo(() => {
    ...
    const liveContent = contentRows
      ...
      .map((c) => {
        const canvas: CanvasType =
          c.kind === "brief" ? "seo-brief"
          : c.kind === "blog" ? "article"
          : c.kind === "email" ? "email"
          : c.kind === "landing" ? "landing-page"
          : "social-post";
        return {
          id: c.id,
          ...
          payload: {
            body: c.body, hashtags: c.hashtags, media_url: c.media_url,
            agent: c.agent, source: "content_items",
    ...
    return MOCK_APPROVALS.flatMap((m, i) => {
      ...
      return [{ ...m, clientId: c.id, clientName: c.name, payload: undefined as Record<string, unknown> | undefined }];
    });
  ...
  const decide = async (id: string, decision: "approved" | "rejected") => {
    if (!canApproveItem(id)) {
      toast.error("You don't have permission", { description: "Only owners, admins, and editors can approve or reject on this client." });
      return;
    }
    setBusyId(id);
    // Optimistic UI
    ...
    try {
      ...
      toast.success(decision === "approved" ? "Approved" : "Rejected", {
        description: decision === "approved" ? "Sent to publish queue." : "Sent back to the brand for revisions.",
      });
    } catch (e: unknown) {
      setResolved((r) => { const n = { ...r }; delete n[id]; return n; });
  ...
  const publishNow = async (id: string) => {
    if (!canApproveItem(id)) {
      toast.error("You don't have permission", { description: "Only owners, admins, and editors can publish on this client." });
      return;
    }
    setBusyId(id);
  ...
  const [bulkConfirm, setBulkConfirm] = useState<null | { kind: "approve" | "reject"; ids: string[] }>(null);
  const requestApproveAll = () => {
    const ids = pendingApprovals.map((a) => a.id).filter(canApproveItem);
    if (ids.length === 0) { toast("Nothing to approve", { description: "No items you have permission to approve." }); return; }
    setBulkConfirm({ kind: "approve", ids });
  };
  const requestRejectAll = () => {
    const ids = pendingApprovals.map((a) => a.id).filter(canApproveItem);
    if (ids.length === 0) { toast("Nothing to reject", { description: "No items you have permission to reject." }); return; }
    setBulkConfirm({ kind: "reject", ids });
  };

  // Revert a bulk decision (server + local) within the undo window.
  const undoBulk = async (ids: string[], prev: Record<string, "approved" | "rejected" | "skipped">) => {
    setResolved((r) => {
      const n = { ...r };
      ids.forEach((id) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("content:changed"));
      window.dispatchEvent(new CustomEvent("approvals:changed"));
      const wsIds = ids.map((id) => combinedApprovals.find((a) => a.id === id)?.clientId).filter(Boolean) as string[];
  ...
  const approveAll = async () => {
    ...
    ids.forEach((id) => { if (resolved[id]) prev[id] = resolved[id]; });
    setBulkBusy(true);
    setResolved((r) => {
      const n = { ...r };
    ...
    try {
      ...
      if (approvalIds.length > 0) {
        const { error } = await supabase
          ...
          .in("id", approvalIds);
        if (error) { toast.error("Some approvals failed", { description: error.message }); return; }
      }
      window.dispatchEvent(new CustomEvent("content:changed"));
      window.dispatchEvent(new CustomEvent("approvals:changed"));
      const wsIdsA = ids.map((id) => combinedApprovals.find((a) => a.id === id)?.clientId).filter(Boolean) as string[];
  ...
  const rejectAll = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("content:changed"));
      window.dispatchEvent(new CustomEvent("approvals:changed"));
      const wsIdsR = ids.map((id) => combinedApprovals.find((a) => a.id === id)?.clientId).filter(Boolean) as string[];
  ...
  const draftWeekForAll = async () => {
    ...
    await Promise.all(targets.map(async (c) => {
      ...
      } catch (e) { console.warn("weekly draft failed for", c.name, e); }
    }));
    setBulkBusy(false);
    window.dispatchEvent(new CustomEvent("content:changed"));
    toast.success(`Drafted ${created} items`, { description: "Fresh approvals across every brand." });
  };
  ...
  const buildDigest = () => {
    ...
    const approvals = pendingApprovals.slice(0, 20).map((a) => ({ client: a.clientName, title: a.title }));
    const recent = combinedRecent.slice(0, 20).map((r) => ({ client: r.clientName, title: r.title, when: r.when }));
  ...
  const digestToText = () => {
    ...
    const lines: string[] = [];
    lines.push(`Raval AI · Weekly digest — ${dateLabel}`);
    lines.push("");
    lines.push(`Clients: ${summary.clients} · Active ${summary.active} · Onboarding ${summary.onboarding}`);
  ...
  const copyDigest = async () => {
    try {
      ...
      toast.success("Weekly digest copied", { description: "Paste it into Slack, email or a doc." });
    } catch {
  ...
  const exportDigestCsv = () => {
    ...
    rows.push(["Summary", "", `Clients ${summary.clients} · Active ${summary.active} · Onboarding ${summary.onboarding}`, ""].map(csvEscape).join(","));
    rows.push(["Summary", "", `Needs approval ${summary.pending} · Scheduled this week ${summary.scheduled}`, ""].map(csvEscape).join(","));
    approvals.forEach((a) => rows.push(["Approval waiting", a.client, a.title, ""].map(csvEscape).join(",")));
    recent.forEach((r) => rows.push(["Recently shipped", r.client, r.title, r.when].map(csvEscape).join(",")));
    downloadBlob(new Blob([rows.join("\n")], { type: "text/csv;charset=utf-8" }), `raval-weekly-digest-${stamp()}.csv`);
    toast.success("CSV downloaded", { description: "Weekly digest saved to your device." });
  };

  const exportDigestPdf = () => {
    const { dateLabel, summary, approvals, recent } = buildDigest();
    const esc = (s: string | undefined | null) => String(s ?? "").replace(/[&<>"']/g, (c) => (({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" } as Record<string, string>)[c] ?? c));
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Raval AI · Weekly digest — ${esc(dateLabel)}</title>
<style>
...
<h1>Raval AI · Weekly digest</h1><div class="sub">${esc(dateLabel)}</div>
<div class="stats">
...
<div class="foot">Generated by Raval AI · ${esc(new Date().toLocaleString())}</div>
<script>window.addEventListener("load",()=>{setTimeout(()=>window.print(),200)});</script>
</body></html>`;
    const w = window.open("", "_blank", "width=900,height=1000");
    if (!w) { toast.error("Popup blocked", { description: "Allow popups to export PDF." }); return; }
    w.document.write(html); w.document.close();
    toast.success("Opening PDF export", { description: "Choose \"Save as PDF\" in the print dialog." });
  };

  // Map a content_items row to a Studio canvas type — same logic StudioRail uses.
  const canvasForRow = (kind: string): CanvasType =>
    kind === "brief" ? "seo-brief"
    : kind === "blog" ? "article"
    : kind === "email" ? "email"
    : kind === "landing" ? "landing-page"
    : "social-post";
  ...
  const combinedScheduled = useMemo(() => {
    ...
    const real = contentRows
      ...
      .map(({ row, ts }) => {
        ...
        const when = isToday ? `Today, ${time}` : d.toLocaleDateString([], { weekday: "short" }) + `, ${time}`;
        return {
          id: row.id,
  ...
  const imageLibMeta = useMemo(() => {
    ...
    const push = (it: { id: string; title: string; clientName: string; channel?: string | null }) => {
      if (!it.id) return;
      map[it.id] = { postId: it.id, title: it.title, clientName: it.clientName, channel: it.channel ?? null };
    };
    combinedApprovals.forEach(push);
    combinedScheduled.forEach(push);
    combinedRecent.forEach(push);
    contentRows.forEach((c) => push({
      id: c.id,
  ...
  const dynamicSuggestions = useMemo(() => {
    const out: { title: string; body: string; icon: React.ReactNode; tint: string; prompt?: string; clientId?: string }[] = [];
    ...
    if (pending.length > 0) {
      out.push({
        ...
        icon: <Bell className="h-3.5 w-3.5" />, tint: "#f59e0b",
      });
    }
    ...
    const idle = clients.filter((c) => c.client_status !== "paused" && !scheduledByClient.has(c.id));
    if (idle.length > 0) {
      const first = idle[0];
      out.push({
        title: `Plan a week for ${first.name}`,
        body: `${idle.length} client${idle.length === 1 ? "" : "s"} ${idle.length === 1 ? "has" : "have"} nothing scheduled.`,
        icon: <Calendar className="h-3.5 w-3.5" />, tint: "#3b82f6",
        clientId: first.id,
    ...
    if (scheduledCount > 0) {
      out.push({
        ...
        icon: <Activity className="h-3.5 w-3.5" />, tint: "#8b5cf6",
      });
    }
    ...
    if (published.length >= 5) {
      out.push({
        ...
        icon: <BarChart3 className="h-3.5 w-3.5" />, tint: "#22c55e",
      });
    }

    if (out.length === 0) {
      out.push(
        { title: "Add your first client", body: "Onboard a brand and Raval drafts a week instantly.", icon: <Plus className="h-3.5 w-3.5" />, tint: "#3b82f6" },
        { title: "Take a coffee break ☕", body: "Inbox zero across every brand. Nice work.", icon: <Coffee className="h-3.5 w-3.5" />, tint: "#22c55e" },
      );
    }
    ...
    <div className="relative min-h-[100dvh] overflow-hidden bg-background text-foreground">
      ...
      <header className="relative z-10 flex h-14 items-center justify-between gap-3 px-5">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <Link to="/projects" aria-label="Raval AI home" className="hidden h-9 shrink-0 items-center sm:flex"><Logo height={30} /></Link>
        </div>
      ...
      >
        ...
        >
          <Sparkles className="h-3 w-3 text-aura" /> Agency HQ
        </motion.div>
        <motion.h1
          variants={itemUp}
        ...
        <motion.p
          variants={itemUp}
          className="mt-2 max-w-xl text-[14px] text-muted-foreground"
        >
        ...
        <motion.div variants={stagger} className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <motion.div variants={itemUp}>
            <SnapshotTile icon={<Inbox className="h-4 w-4" />} label="Need approval" value={todayApprovals} tint="#f59e0b" onClick={() => document.getElementById("today")?.scrollIntoView({ behavior: "smooth" })} />
          </motion.div>
          <motion.div variants={itemUp}>
            <SnapshotTile icon={<Calendar className="h-4 w-4" />} label="Scheduled today" value={todayPosts} tint="#3b82f6" />
          </motion.div>
          <motion.div variants={itemUp}>
            <SnapshotTile icon={<ListTodo className="h-4 w-4" />} label="Open tasks" value={todayTasks} tint="#8b5cf6" />
          </motion.div>
          <motion.div variants={itemUp}>
            <SnapshotTile icon={<Users className="h-4 w-4" />} label="Active clients" value={activeCount} sub={onboardingCount > 0 ? `${onboardingCount} onboarding` : undefined} tint="#22c55e" />
          </motion.div>
        </motion.div>
        ...
        <motion.div variants={itemUp} className="mt-4 flex flex-wrap items-center gap-2">
          <QuickAction
            icon={<Zap className="h-3.5 w-3.5" />} label="Draft this week for every client"
            hint="Auto-fills empty calendars" onClick={draftWeekForAll} busy={bulkBusy} tint="#f59e0b"
          />
          {(() => {
            ...
            return (
              <>
                <QuickAction
                  icon={<Check className="h-3.5 w-3.5" />} label={`Approve ${approvableCount}`}
                  onClick={requestApproveAll} busy={bulkBusy} tint="#22c55e"
                />
                <QuickAction
                  icon={<XIcon className="h-3.5 w-3.5" />} label="Reject all"
                  onClick={requestRejectAll} busy={bulkBusy} tint="#f43f5e"
                />
              </>
            );
          })()}
          <QuickAction
            icon={<Copy className="h-3.5 w-3.5" />} label="Copy weekly digest"
            hint="For Slack or email" onClick={copyDigest} tint="#3b82f6"
          />
          <QuickAction
            icon={<Upload className="h-3.5 w-3.5 rotate-180" />} label="Export digest CSV"
            hint="Spreadsheet-ready" onClick={exportDigestCsv} tint="#0ea5e9"
          />
          <QuickAction
            icon={<Upload className="h-3.5 w-3.5 rotate-180" />} label="Export digest PDF"
            hint="Print or save" onClick={exportDigestPdf} tint="#ef4444"
          />
          <QuickAction
            icon={<ImageIcon className="h-3.5 w-3.5" />} label="Image library"
            hint="All generated visuals" onClick={() => setImageLibOpen(true)} tint="#14b8a6"
          />
          <QuickAction
            icon={<CommandIcon className="h-3.5 w-3.5" />} label="Jump to anything"
            onClick={() => setCmdOpen(true)} tint="#8b5cf6"
          />
        </motion.div>
      </motion.section>
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          ...
          <div className="space-y-5 lg:col-span-2">
            <Card
              ...
              action={pendingApprovals.filter((a) => canApproveItem(a.id)).length > 1 ? (
                <Button
            ...
            >
              {clients.length > 1 && pendingAllCount > 0 && (
                <div className="mb-3 -mt-1 flex items-center gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  ...
                  <FilterChip active={approvalsFilter === "all"} onClick={() => setApprovalsFilter("all")} label={`All · ${pendingAllCount}`} />
                  {clients.map((c) => {
                    const count = combinedApprovals.filter((a) => !resolved[a.id] && a.clientId === c.id).length;
              ...
              {pendingApprovals.length === 0 ? (
                autoGenBusy ? (
                  <EmptyLine icon={<Loader2 className="h-4 w-4 animate-spin text-aura" />} text="Drafting fresh on-brand content for every client…" />
                ) : (
                  <EmptyLine icon={<CheckCircle2 className="h-4 w-4 text-emerald-500" />} text="Inbox zero across every client. Nice." />
                )
              ) : (
                      ...
                      >
                        <button
                          onClick={() => setPreviewItem({
                            id: it.id, title: it.title, clientName: it.clientName,
                            channel: it.channel, canvas: it.canvas as CanvasType, payload: it.payload,
                          })}
                        ...
                        >
                          <span
                            ...
                            style={{ background: `linear-gradient(135deg, ${color}24, ${color}08)`, boxShadow: `inset 0 0 0 1px ${color}30` }}
                          >
                            {tile?.icon && <tile.icon className="h-4 w-4" style={{ color }} strokeWidth={2.25} />}
                          </span>
                        ...
                        <div className="flex shrink-0 items-center gap-1">
                          {canApproveItem(it.id) ? (
                            <>
                              ...
                              >
                                {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <XIcon className="h-3.5 w-3.5" />}
                              </button>
                              <button
            ...
            <Card title="Today & this week" icon={<Calendar className="h-3.5 w-3.5 text-[#3b82f6]" />} hint="Combined schedule">
                  ...
                  .map((it) => (
                    <ScheduleRow
                      ...
                      onPreview={() => setPreviewItem({
                        id: it.id, title: it.title, clientName: it.clientName,
                        channel: it.channel, canvas: it.canvas as CanvasType,
                      })}
                      onReschedule={() => toast.success("Rescheduled", { description: `${it.title} moved to tomorrow.` })}
                      onSkip={() => {
            ...
            <Card title="What to do next" icon={<Lightbulb className="h-3.5 w-3.5 text-aura" />} hint="Based on live data">
          ...
          <div className="space-y-5">
            <Card title="Recent activity" icon={<Activity className="h-3.5 w-3.5 text-[#8b5cf6]" />}>
                  ...
                  return (
                    <li key={it.id}>
                      ...
                      >
                        ...
                        <span className="flex-1 truncate text-[12.5px] text-foreground/85">{it.title}</span>
                        <span className="hidden shrink-0 text-[10.5px] text-muted-foreground sm:inline">{it.clientName}</span>
                        <span className="shrink-0 text-[10.5px] text-muted-foreground/70">{it.when}</span>
                      </button>
                    </li>
                  );
                })}
            ...
            <Card title="Your clients" icon={<Layers className="h-3.5 w-3.5 text-[#0ea5e9]" />} hint={`${clients.length}`}>
              {clients.length === 0 ? (
                <EmptyLine icon={<Plus className="h-4 w-4" />} text="No clients yet — add your first brand." />
              ) : (
                <ul className="space-y-1">
                  {clients.slice(0, 6).map((c) => (
                    <li key={c.id}>
                      ...
                      >
                        ...
                        <span className="flex-1 truncate text-[12.5px] font-medium text-foreground/90">{c.name}</span>
            ...
            <Card title="Client health" icon={<Activity className="h-3.5 w-3.5 text-emerald-500" />} hint="Live signals">
              {clients.length === 0 ? (
                <EmptyLine icon={<Plus className="h-4 w-4" />} text="Add a client to see health signals." />
              ) : (
                <ul className="space-y-1.5">
                  {clients.slice(0, 6).map((c) => {
                    const pending = contentRows.filter((r) => r.workspace_id === c.id && (r.status === "pending" || r.status === "draft")).length;
                    const scheduled = contentRows.filter((r) => r.workspace_id === c.id && r.status === "scheduled").length;
                    const published = contentRows.filter((r) => r.workspace_id === c.id && r.status === "published").length;
                    let tone: "good" | "warn" | "risk" = "good";
                    let label = "On track";
                    if (c.client_status === "paused") { tone = "warn"; label = "Paused"; }
                    else if (scheduled === 0 && published === 0) { tone = "risk"; label = "At risk"; }
                    else if (pending >= 4) { tone = "warn"; label = "Approvals piling"; }
                    else if (scheduled < 2) { tone = "warn"; label = "Thin schedule"; }
                    const dot = tone === "good" ? "bg-emerald-500" : tone === "warn" ? "bg-amber-500" : "bg-rose-500";
                    ...
                    return (
                      <li key={c.id}>
                        ...
                        >
                          ...
                          <span className="min-w-0 flex-1 truncate text-[12.5px] font-medium text-foreground/90">{c.name}</span>
                          <span className="hidden shrink-0 text-[10.5px] tabular-nums text-muted-foreground sm:inline">
                            {scheduled} sched · {pending} pend
                          </span>
                          <span className={cn("shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-medium", chip)}>{label}</span>
                        </button>
                      </li>
                    );
                  })}
      ...
      <PreviewSheet
        ...
        onApprove={(id) => { decide(id, "approved"); setPreviewItem(null); }}
        onReject={(id) => { decide(id, "rejected"); setPreviewItem(null); }}
        onOpenClient={(id) => { openClient(id); setPreviewItem(null); }}
      ...
      <CommandPalette
        ...
        onOpenClient={(id) => { setCmdOpen(false); openClient(id); }}
        onScrollTo={(id) => { setCmdOpen(false); document.getElementById(id)?.scrollIntoView({ behavior: "smooth" }); }}
        actions={{
          approveAll: () => { setCmdOpen(false); requestApproveAll(); },
          rejectAll: () => { setCmdOpen(false); requestRejectAll(); },
          draftWeek: () => { setCmdOpen(false); draftWeekForAll(); },
          copyDigest: () => { setCmdOpen(false); copyDigest(); },
          allClients: () => { setCmdOpen(false); navigate({ to: "/projects" }); },
        }}
      />
...
function BulkConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={onCancel}>
      <motion.div
      ...
      >
        <div className="flex items-start gap-3 p-5">
          ...
          <div className="min-w-0">
            <h3 className="text-[15px] font-semibold text-foreground">
              {isApprove ? `Approve ${count} item${count === 1 ? "" : "s"}?` : `Reject ${count} item${count === 1 ? "" : "s"}?`}
            </h3>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border/60 bg-background/40 px-4 py-3">
          ...
          >
            {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : isApprove ? <Check className="h-3.5 w-3.5" /> : <XIcon className="h-3.5 w-3.5" />}
...
function QuickAction({
  icon, label, hint, onClick, busy, tint,
}: { icon: React.ReactNode; label: string; hint?: string; onClick: () => void; busy?: boolean; tint: string }) {
  return (
    <motion.button
      whileHover={{ y: -1 }}
      whileTap={{ scale: 0.98 }}
    ...
    >
      <span className="grid h-6 w-6 place-items-center rounded-full transition group-hover:scale-105" style={{ background: `${tint}22`, color: tint }}>
        {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : icon}
      </span>
      <span className="truncate">{label}</span>
      {hint && <span className="hidden text-[10.5px] text-muted-foreground md:inline">· {hint}</span>}
    </motion.button>
  );
}
...
function CommandPalette({
  open, onClose, clients, pendingCount, onOpenClient, onScrollTo, actions,
}: {
  open: boolean;
  onClose: () => void;
  clients: Client[];
  ...
  const items: { key: string; label: string; hint?: string; icon: React.ReactNode; run: () => void; group: string }[] = [
    { key: "act-approve", group: "Bulk actions", label: `Approve all pending (${pendingCount})`, icon: <Check className="h-3.5 w-3.5 text-emerald-500" />, run: actions.approveAll },
    { key: "act-reject", group: "Bulk actions", label: "Reject all pending", icon: <XIcon className="h-3.5 w-3.5 text-rose-500" />, run: actions.rejectAll },
    { key: "act-draft", group: "Bulk actions", label: "Draft this week for every client", hint: "AI · Spark", icon: <Zap className="h-3.5 w-3.5 text-amber-500" />, run: actions.draftWeek },
    { key: "act-digest", group: "Bulk actions", label: "Copy weekly digest", icon: <Copy className="h-3.5 w-3.5 text-[#3b82f6]" />, run: actions.copyDigest },
    { key: "nav-today", group: "Navigate", label: "Jump to today's queue", icon: <Inbox className="h-3.5 w-3.5" />, run: () => onScrollTo("today") },
    { key: "nav-projects", group: "Navigate", label: "All clients", icon: <Users className="h-3.5 w-3.5" />, run: actions.allClients },
    ...clients.map((c) => ({
      key: `client-${c.id}`,
      group: "Open client",
      label: c.name,
  ...
  const filtered = norm
    ? items.filter((i) => (i.label + " " + (i.hint ?? "") + " " + i.group).toLowerCase().includes(norm))
    : items;
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center bg-black/40 p-4 pt-[10vh] backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center gap-2 border-b border-border/60 px-3 py-2.5">
          ...
          />
          <kbd className="hidden h-5 items-center gap-0.5 rounded-md border border-border/60 bg-background/70 px-1.5 text-[10px] font-semibold text-foreground/70 sm:inline-flex">Esc</kbd>
        </div>
            ...
            Object.entries(grouped).map(([group, list]) => (
              <div key={group} className="mb-1">
                ...
                {list.map((it) => (
                  ...
                  >
                    <span className="grid h-6 w-6 place-items-center rounded-md bg-background/70">{it.icon}</span>
                    <span className="flex-1 truncate">{it.label}</span>
                    {it.hint && <span className="truncate text-[11px] text-muted-foreground">{it.hint}</span>}
                  </button>
                ))}
              </div>
            ))
...
function SnapshotTile({
  icon, label, value, sub, tint, onClick,
}: { icon: React.ReactNode; label: string; value: number; sub?: string; tint: string; onClick?: () => void }) {
    ...
    <div className="relative group">
      ...
      >
        ...
        <div className="mt-3 flex items-baseline gap-2">
          <span className="font-display text-[30px] font-light leading-none tracking-tight text-foreground">{value}</span>
        </div>
        {sub && (
          <div className="mt-3 flex items-center text-[11px] text-muted-foreground">
            <span className="mr-1 font-semibold" style={{ color: tint }}>{sub.split(" ")[0]}</span>
            {sub.split(" ").slice(1).join(" ")}
          </div>
        )}
        {!sub && (
          <div className="mt-3 text-[11px] text-muted-foreground/70">Live</div>
        )}
      </Comp>
    </div>
  );
...
function SectionHeading({
  ...
  return (
    ...
    >
      <div>
        <div className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-aura">{eyebrow}</div>
        <h2 className="font-display mt-1 text-[22px] font-normal italic leading-tight tracking-tight sm:text-[26px]">{title}</h2>
...
function Card({
  title, icon, hint, action, children,
}: { title: string; icon?: React.ReactNode; hint?: string; action?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-[#0d0d0d]/90 p-5 backdrop-blur-xl sm:p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {hint && <span className="text-[10.5px] uppercase tracking-wider text-muted-foreground/80">{hint}</span>}
          {action}
        </div>
      </div>
      {children}
...
function ScheduleRow({
  item, onPreview, onReschedule, onSkip,
}: {
  ...
  return (
    <li className="group flex items-center gap-3 rounded-xl px-2 py-2 transition hover:bg-secondary/60">
      <button onClick={onPreview} className="flex min-w-0 flex-1 items-center gap-3 text-left">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg" style={{ background: `${color}1a`, color }}>
...
function PreviewSheet({
  item, onClose, onApprove, onReject, onOpenClient, clientById,
}: {
  item: null | { id: string; title: string; clientName: string; channel?: string; canvas: CanvasType; payload?: Record<string, unknown> };
  onClose: () => void;
  ...
  const color = TINT[tile?.tint ?? "brand-blue"];
  const caption = item?.payload && typeof item.payload.caption === "string" ? (item.payload.caption as string) : null;
  const summary = item?.payload && typeof item.payload.summary === "string" ? (item.payload.summary as string) : null;
  const bodyFromPayload = item?.payload && typeof item.payload.body === "string" ? (item.payload.body as string) : null;
  const clientId = item ? (Array.from(clientById.values()).find((c) => c.name === item.clientName)?.id ?? "") : "";
  const initialMedia = item?.payload && typeof item.payload.media_url === "string" ? (item.payload.media_url as string) : null;
  const [customImage, setCustomImage] = useState<string | null>(initialMedia);
  const [dragOver, setDragOver] = useState(false);
  const [savingImg, setSavingImg] = useState(false);
  useEffect(() => { setCustomImage(initialMedia); }, [initialMedia, item?.id]);
            ...
            <SheetHeader className="border-b border-border/60 p-5">
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{ background: `linear-gradient(135deg, ${color}24, ${color}08)`, boxShadow: `inset 0 0 0 1px ${color}30` }}
                >
                  {tile?.icon && <tile.icon className="h-5 w-5" style={{ color }} strokeWidth={2.25} />}
                </span>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              ...
              >
                ...
                {customImage && (
                  <div className="mb-3 overflow-hidden rounded-xl border border-border/60">
                    <img src={customImage} alt="Attached" className="block max-h-72 w-full object-cover" />
                  </div>
                )}
                <CanvasPreview
                  canvas={item.canvas}
              ...
              <div>
                <div className="mb-1.5 flex items-center justify-between">
                  ...
                  {savingImg && <span className="text-[10.5px] text-muted-foreground">Saving…</span>}
                </div>
                <label
                  ...
                  className={cn(
                    ...
                    dragOver ? "border-emerald-400/70 bg-emerald-500/5" : "border-border/70 hover:bg-muted/40",
                  )}
                >
                  <span className="grid h-10 w-10 place-items-center rounded-lg bg-muted text-muted-foreground group-hover:text-foreground">
                    {customImage ? <ImagePlus className="h-4 w-4" /> : <Upload className="h-4 w-4" />}
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="text-[12.5px] font-medium text-foreground">
                      {customImage ? "Replace image" : "Upload or drop an image"}
                    </div>
                    <div className="text-[11px] text-muted-foreground">PNG · JPG · WebP — up to 8 MB</div>
                  </div>
                  {customImage && (
                    <button
                      type="button"
              ...
              <div>
                ...
                <div className="rounded-xl border border-border/60 bg-background/60 p-3 text-[12.5px] text-foreground/85">
                  This sits inside <span className="font-medium text-foreground">{item.clientName}</span>'s active calendar.
                  Approving sends it to the publish queue; rejecting returns it to the brand workspace with notes.
                </div>
              </div>
            </div>
...
function CanvasPreview({
  canvas, channel, title, caption, summary, clientName, color,
}: {
  canvas: CanvasType;
  channel?: string;
  title: string;
  ...
  const body = caption ?? summary ?? "AI-drafted content awaiting your review — tap Approve to push it live, or Reject to send notes back to the brand workspace.";
  const handle = "@" + clientName.toLowerCase().replace(/[^a-z0-9]+/g, "").slice(0, 18);
  const brandLetter = clientName.trim().charAt(0).toUpperCase() || "·";
  const hashtags = ["#brand", "#marketing", "#growth", "#" + clientName.toLowerCase().replace(/[^a-z0-9]+/g, "").slice(0, 12)]
  ...
  const rise: Variants = {
    hidden: { opacity: 0, y: 8 },
    show: (i: number = 0) => ({ opacity: 1, y: 0, transition: { delay: 0.05 * i, duration: 0.35, ease: [0.22, 1, 0.36, 1] } }),
  };
  ...
  const Avatar = (
    <span
      ...
      style={{ background: `linear-gradient(135deg, ${color}, color-mix(in oklab, ${color} 60%, #000))` }}
    >
      {brandLetter}
    </span>
  );
        ...
        <motion.div initial="hidden" animate="show" className={shell}>
          <motion.div variants={rise} custom={0} className="flex items-center gap-3 px-4 pt-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[11.5px] text-muted-foreground">2h · <Globe className="inline h-3 w-3" /></div>
            </div>
            <span className="rounded-md bg-[#0a66c2] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">in</span>
          </motion.div>
          <motion.p variants={rise} custom={1} className={`px-4 pt-3 pb-3 ${bodyType}`}>
            {body}
            <span className="mt-1.5 block text-[12.5px] font-medium text-[#0a66c2]">{hashtags}</span>
          </motion.p>
          <motion.div variants={rise} custom={2} className="relative mx-4 mb-3 aspect-[1.91/1] overflow-hidden rounded-xl" style={{ background: `linear-gradient(135deg, ${color}, color-mix(in oklab, ${color} 55%, #000))` }}>
            <div className="absolute inset-0 grid place-items-center px-6 text-center text-[15px] font-semibold leading-snug text-white drop-shadow-md">{title}</div>
          </motion.div>
          <motion.div variants={rise} custom={3} className="flex items-center justify-between px-4 pb-2 text-[11.5px] text-muted-foreground">
            <span className="inline-flex items-center gap-1"><span className="inline-grid h-4 w-4 place-items-center rounded-full bg-[#0a66c2] text-[9px] text-white">👍</span> 1,284</span>
            <span>87 comments · 32 reposts</span>
          </motion.div>
          <motion.div variants={rise} custom={4} className="flex items-center justify-around border-t border-border/70 py-2 text-[12px] font-medium text-muted-foreground">
        ...
        <motion.div initial="hidden" animate="show" className={`${shell} p-4`}>
          <motion.div variants={rise} custom={0} className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <motion.p variants={rise} custom={1} className={`mt-1.5 whitespace-pre-line ${bodyType}`}>
                {body}
                <span className="mt-1.5 block text-[13px] font-medium text-[#1d9bf0]">{hashtags}</span>
              </motion.p>
              <motion.div variants={rise} custom={2} className="mt-3 flex items-center gap-6 text-[12px] font-medium text-muted-foreground">
                <span className="inline-flex items-center gap-1.5"><MessageCircle className="h-4 w-4" /> 142</span>
                <span className="inline-flex items-center gap-1.5"><Repeat2 className="h-4 w-4" /> 386</span>
                <span className="inline-flex items-center gap-1.5 text-rose-500"><Heart className="h-4 w-4 fill-rose-500" /> 2,148</span>
                <span className="inline-flex items-center gap-1.5"><BarChart3 className="h-4 w-4" /> 24.1k</span>
              </motion.div>
            </div>
          </motion.div>
        </motion.div>
    ...
    return (
      <motion.div initial="hidden" animate="show" className={shell}>
        <motion.div variants={rise} custom={0} className="flex items-center gap-3 p-3">
          <span className="rounded-full p-[2px]" style={{ background: "conic-gradient(from 180deg, #feda75, #fa7e1e, #d62976, #962fbf, #4f5bd5, #feda75)" }}>
        ...
        <motion.div variants={rise} custom={1} className="relative aspect-square w-full overflow-hidden" style={{ background: `linear-gradient(135deg, ${color}, ${color}55)` }}>
        ...
        <motion.div variants={rise} custom={2} className="flex items-center gap-4 px-4 pt-3 text-foreground">
          <Heart className="h-6 w-6 fill-rose-500 text-rose-500" /><MessageCircle className="h-6 w-6" /><Send className="h-6 w-6" />
          <Bookmark className="ml-auto h-5 w-5" />
        </motion.div>
        <motion.div variants={rise} custom={3} className="px-4 pb-1 pt-2 text-[12.5px] font-semibold text-foreground">12,482 likes</motion.div>
        ...
        <motion.p variants={rise} custom={5} className="px-4 pb-1 text-[12.5px] font-medium text-[#385898]">{hashtags}</motion.p>
        <motion.p variants={rise} custom={6} className="px-4 pb-4 pt-1 text-[11px] uppercase tracking-wide text-muted-foreground">2 hours ago</motion.p>
      </motion.div>
    );
  }
  ...
  if (canvas === "email") {
    return (
      <motion.div initial="hidden" animate="show" className={shell}>
        <motion.div variants={rise} custom={0} className="border-b border-border/70 bg-muted/50 px-4 py-3 text-[12px]">
          <div className="flex items-center justify-between text-muted-foreground">
            <span>From <span className="font-medium text-foreground">{clientName} &lt;hello@brand.com&gt;</span></span>
            <span>Today</span>
          </div>
          <div className="mt-1 text-foreground"><span className="text-muted-foreground">Subject: </span><span className="font-semibold">{title}</span></div>
        </motion.div>
        <motion.div variants={rise} custom={1} className="space-y-3 p-5">
          <h4 className="text-[18px] font-semibold leading-snug tracking-tight text-foreground">{title}</h4>
          <p className={bodyType}>{body}</p>
          <p className={bodyType}>
            Here's what's new this week — handpicked stories, tips, and updates from the {clientName} team to help you get the most out of every moment.
          </p>
          <motion.button
            variants={rise} custom={2}
  ...
  if (canvas === "article") {
    return (
      <motion.div initial="hidden" animate="show" className={shell}>
        <motion.div variants={rise} custom={0} className="relative aspect-[2/1] w-full" style={{ background: `linear-gradient(135deg, ${color}66, ${color}15)` }}>
          <span
        ...
        <div className="space-y-3 p-5">
          <motion.div variants={rise} custom={1} className={subType}>4 min read · Published today</motion.div>
          <motion.h4 variants={rise} custom={2} className="text-[18px] font-semibold leading-snug tracking-tight text-foreground">{title}</motion.h4>
          <motion.p variants={rise} custom={3} className={bodyType}>{body}</motion.p>
          <motion.div variants={rise} custom={4} className="flex items-center gap-2.5 border-t border-border/60 pt-3 text-[12px] text-muted-foreground">
            {Avatar}
            <span>by <span className="font-medium text-foreground">Spark</span> · for <span className="font-medium text-foreground">{clientName}</span></span>
          </motion.div>
        </div>
      </motion.div>
    );
  ...
  if (canvas === "seo-brief") {
    return (
      <motion.div initial="hidden" animate="show" className={`${shell} p-5`}>
        <motion.div variants={rise} custom={0} className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.1em]" style={{ color }}>
          <SearchIcon className="h-3.5 w-3.5" /> SEO brief · {clientName}
        </motion.div>
        <motion.h4 variants={rise} custom={1} className="mt-2 text-[17px] font-semibold leading-snug tracking-tight text-foreground">{title}</motion.h4>
        <motion.p variants={rise} custom={2} className={`mt-2 ${bodyType}`}>{body}</motion.p>
        <motion.div variants={rise} custom={3} className="mt-4 grid grid-cols-3 gap-2.5">
          {[
            ...
            <div key={m.k} className="rounded-xl border border-border/70 bg-muted/40 px-3 py-2.5 text-center">
        ...
        <motion.div variants={rise} custom={4} className="mt-4">
          <div className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Target keywords</div>
          <div className="flex flex-wrap gap-1.5">
            {["how to","compare","pricing","alternatives","reviews"].map((t) => (
              <span key={t} className="rounded-full border border-border/70 bg-background px-2.5 py-1 text-[11.5px] font-medium text-foreground">{t}</span>
            ))}
          </div>
        </motion.div>
      </motion.div>
  ...
  if (canvas === "landing-page") {
    return (
      <motion.div initial="hidden" animate="show" className={shell}>
        <motion.div variants={rise} custom={0} className="flex items-center gap-1.5 border-b border-border/70 bg-muted/50 px-3 py-2">
          <span className="h-2.5 w-2.5 rounded-full bg-rose-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
          <span className="ml-2 truncate rounded-md border border-border/60 bg-background px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">{clientName.toLowerCase().replace(/\s+/g, "")}.com</span>
        </motion.div>
        <div className="space-y-4 p-5">
          <motion.h4 variants={rise} custom={1} className="text-[18px] font-semibold leading-tight tracking-tight text-foreground">{title}</motion.h4>
          <motion.p variants={rise} custom={2} className={bodyType}>{body}</motion.p>
          <motion.div variants={rise} custom={3} className="flex items-center gap-2 pt-1">
            <span className="rounded-lg px-3.5 py-2 text-[12.5px] font-semibold text-white shadow-sm" style={{ background: color }}>Get started</span>
            <span className="rounded-lg border border-border bg-background px-3.5 py-2 text-[12.5px] font-medium text-foreground">Learn more</span>
          </motion.div>
          <motion.div variants={rise} custom={4} className="grid grid-cols-3 gap-2.5 pt-2">
            {[0,1,2].map((i) => (
              <div key={i} className="aspect-square rounded-lg border border-border/60" style={{ background: `linear-gradient(135deg, ${color}30, ${color}08)` }} />
            ))}
          </motion.div>
        </div>
      </motion.div>
  ...
  return (
    <motion.div initial="hidden" animate="show" className={`${shell} p-5`}>
      <motion.div variants={rise} custom={0} className="aspect-video w-full rounded-xl" style={{ background: `conic-gradient(from 90deg at 50% 50%, ${color}, ${color}30, ${color})` }} />
      <motion.h4 variants={rise} custom={1} className="mt-4 text-[16px] font-semibold leading-snug tracking-tight text-foreground">{title}</motion.h4>
      <motion.p variants={rise} custom={2} className={`mt-1.5 ${bodyType}`}>{body}</motion.p>
    </motion.div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-background/60 p-2.5">
      ...
      <div className="mt-0.5 flex items-baseline justify-between">
        ...
        {delta && (
          <span className={cn("inline-flex items-center gap-0.5 text-[10.5px] font-medium",
...
const SUGGESTIONS: { title: string; body: string; icon: React.ReactNode; tint: string }[] = [
  { title: "Refresh weekly schedules", body: "3 clients have nothing scheduled past Friday.", icon: <Calendar className="h-3.5 w-3.5" />, tint: "#3b82f6" },
  { title: "Approve LinkedIn batch", body: "Echo drafted 5 posts — same brand voice.", icon: <Bell className="h-3.5 w-3.5" />, tint: "#f59e0b" },
  { title: "Run a content audit", body: "Spark spotted 7 drafts that haven't moved in 14d.", icon: <Activity className="h-3.5 w-3.5" />, tint: "#8b5cf6" },
  { title: "Take a coffee break ☕", body: "You closed 12 approvals yesterday. Nice.", icon: <Coffee className="h-3.5 w-3.5" />, tint: "#22c55e" },
];

function Suggestion({ title, body, icon, tint, onClick }: { title: string; body: string; icon: React.ReactNode; tint: string; onClick?: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span className="grid h-6 w-6 place-items-center rounded-md" style={{ background: `${tint}1f`, color: tint }}>
          {icon}
        </span>
...
function AuroraBackdrop() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-0 overflow-hidden">
      <div className="absolute -top-32 left-1/2 h-[520px] w-[1100px] -translate-x-1/2 rounded-[50%] opacity-70 blur-3xl"
           style={{ background: "radial-gradient(closest-side, hsl(var(--aura)/0.45), transparent 70%)" }} />
      <div className="absolute top-40 left-[6%] h-[420px] w-[520px] rounded-full opacity-60 blur-3xl"
           style={{ background: "radial-gradient(closest-side, hsl(var(--aura-pink)/0.45), transparent 70%)" }} />
      <div className="absolute top-24 right-[4%] h-[460px] w-[560px] rounded-full opacity-60 blur-3xl"
           style={{ background: "radial-gradient(closest-side, hsl(var(--aura-purple)/0.45), transparent 70%)" }} />
...
const CHANNELS: ChannelRow[] = [
  { key: "instagram", label: "Instagram", posts: 42, reach: 68400, engagement: 7.2, delta: 14, color: "#E1306C" },
  { key: "linkedin",  label: "LinkedIn",  posts: 28, reach: 51200, engagement: 5.4, delta: 9,  color: "#0A66C2" },
  { key: "x",         label: "X",         posts: 36, reach: 32800, engagement: 3.1, delta: -4, color: "#0F172A" },
  { key: "tiktok",    label: "TikTok",    posts: 14, reach: 22100, engagement: 9.6, delta: 22, color: "#000000" },
  { key: "youtube",   label: "YouTube",   posts: 6,  reach: 9800,  engagement: 4.8, delta: 6,  color: "#FF0000" },
  { key: "reddit",    label: "Reddit",    posts: 11, reach: 6200,  engagement: 2.4, delta: -2, color: "#FF4500" },
];
...
function AllClientsAnalytics({
  clients, onOpenClient,
}: { clients: Client[]; onOpenClient: (id: string) => void }) {
  ...
  const pool = clients.length > 0 ? clients : [
  ...
  return (
    <div className="mb-5 rounded-3xl border border-border/70 bg-card/70 p-4 backdrop-blur-xl sm:p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-[10.5px] text-muted-foreground">Last 14 days · combined across {pool.length} {pool.length === 1 ? "client" : "clients"}</span>
      </div>

      {/* KPI row */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KpiCard icon={<Eye className="h-3.5 w-3.5" />} label="Total reach" value={fmt(totalReach)} delta="+12.4%" up tint="#3b82f6"
                 spark={TREND_14D.map((d) => ({ v: d.reach }))} />
        <KpiCard icon={<Heart className="h-3.5 w-3.5" />} label="Avg engagement" value={`${avgEng}%`} delta="+0.6pt" up tint="#E1306C"
                 spark={TREND_14D.map((d) => ({ v: d.engagement }))} />
        <KpiCard icon={<Activity className="h-3.5 w-3.5" />} label="Posts shipped" value={String(totalPosts)} delta="+9" up tint="#8b5cf6"
                 spark={TREND_14D.map((d, i) => ({ v: 2 + (i % 5) }))} />
        <KpiCard icon={<MousePointerClick className="h-3.5 w-3.5" />} label="Link clicks" value="3,284" delta="+18%" up tint="#22c55e"
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-2xl border border-border/60 bg-background/60 p-3.5">
          <div className="mb-2 flex items-center justify-between">
            <div className="text-[11.5px] font-medium text-foreground/80">Reach & engagement trend</div>
            <div className="flex items-center gap-3 text-[10.5px] text-muted-foreground">
              <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[#3b82f6]" /> Reach</span>
              <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[#E1306C]" /> Engagement</span>
            </div>
          </div>
          <div className="h-[180px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={TREND_14D} margin={{ top: 6, right: 4, left: -22, bottom: -4 }}>
                ...
                />
                <Area type="monotone" dataKey="reach" stroke="#3b82f6" strokeWidth={2} fill="url(#reachFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
                ...
                <li key={c.id}>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="shrink-0 text-[11px] tabular-nums text-foreground/80">{fmt(c.reach)}</span>
                      <span className={cn(
      ...
      <div className="mt-4 rounded-2xl border border-border/60 bg-background/60 p-3.5">
        <div className="mb-3 flex items-center justify-between">
          <div className="text-[11.5px] font-medium text-foreground/80">Per-channel performance</div>
          <span className="text-[10px] text-muted-foreground">Reach · engagement · vs prev 14d</span>
        </div>
          ...
          {CHANNELS.map((ch) => {
            ...
            return (
              <div key={ch.key} className="rounded-xl border border-border/50 bg-card/50 p-2.5">
                <div className="flex items-center gap-2">
                  <span className="grid h-7 w-7 place-items-center rounded-lg" style={{ background: `${ch.color}14` }}>
                  ...
                  )}>{ch.delta >= 0 ? "+" : ""}{ch.delta}%</span>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <div>
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Reach</div>
                    <div className="text-[13px] font-semibold tabular-nums">{fmt(ch.reach)}</div>
                  </div>
                  <div>
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Eng.</div>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <Link to="/app" className="inline-flex items-center gap-1 text-[12px] font-medium text-foreground/80 hover:text-foreground">
