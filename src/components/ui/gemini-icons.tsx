...
const SIZE_MAP: Record<string, number> = {
  // xs bucket (16px)
  "h-2": ICON_XS, "h-2.5": ICON_XS, "h-3": ICON_XS, "h-3.5": ICON_XS, "h-4": ICON_XS,
  // sm bucket — default (18px)
  "h-4.5": ICON_SM,
  // md bucket (20px)
  "h-5": ICON_MD, "h-6": ICON_MD,
  // lg bucket (24px)
  "h-7": ICON_LG, "h-8": ICON_LG, "h-9": ICON_LG, "h-10": ICON_LG,
  // oversized (empty states / illustrations) keep their footprint
  "h-11": 28, "h-12": 32, "h-14": 36, "h-16": 40, "h-20": 48,
};
...
function make(name: string, opts: { filled?: boolean; weight?: "regular" | "medium" | "bold" } = {}) {
  const Component = React.forwardRef<HTMLSpanElement, IconProps>(function GeminiIcon(
    props,
    ref,
  ) {
    const {
      className, style, size, strokeWidth: _sw,
      fill, stroke: _stroke, color,
      onCopy: _oc, onCut: _ocu, onPaste: _op,
      ...rest
    } = props;
    ...
    const [glyph, setGlyph] = React.useState<string>(
      cached === false ? FALLBACK_GLYPH : name,
    );
    ...
    return (
      <span
        ...
        style={{ fontSize: `${px}px`, width: `${px}px`, height: `${px}px`, lineHeight: `${px}px`, display: "inline-flex", alignItems: "center", justifyContent: "center", overflow: "hidden", color: tint, ...style }}
        {...spanProps}
      >
        {glyph}
      </span>
