  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative flex h-[85vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl border border-border/70 bg-card/95 shadow-2xl"
      >
        {/* Header */}
        <div className="flex items-center justify-between gap-3 border-b border-border/60 px-5 py-4">
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            className="grid h-9 w-9 place-items-center rounded-full border border-border/60 bg-secondary/60 text-muted-foreground transition hover:border-border hover:bg-secondary hover:text-foreground"
          >
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              className="h-9 w-full rounded-full border border-input bg-background pl-9 pr-3 text-[12.5px] text-foreground placeholder:text-muted-foreground/70 focus:border-ring focus:outline-none"
            />
          </div>
                ...
                {filtered.map((e) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className="group relative flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-background/60 text-left transition hover:border-border hover:bg-secondary/40"
                    >
                      <div className="relative aspect-square w-full overflow-hidden bg-black/40">
                        ...
                        />
                        <span className="absolute right-2 top-2 rounded-full bg-foreground/80 px-2 py-0.5 text-[10px] font-medium text-background backdrop-blur">
                          {SIZE_LABEL[e.size]}
                        </span>
                      </div>
          ...
          {selected && (
            ...
            >
              <motion.div
                ...
                className="grid w-full max-w-4xl grid-cols-1 gap-5 overflow-hidden rounded-2xl border border-border/70 bg-card p-5 md:grid-cols-[1.4fr_1fr]"
              >
                ...
                <div className="flex min-w-0 flex-col">
                  <div className="mb-3 flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="grid h-8 w-8 place-items-center rounded-full border border-border/60 bg-secondary/60 text-muted-foreground hover:bg-secondary hover:text-foreground"
                    >
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "border-border/60 bg-secondary/60 text-muted-foreground hover:border-border hover:text-foreground",
      )}
    >
      {label}
      <span
        className={cn(
          ...
          active ? "bg-background/15" : "bg-background/70",
        )}
      >
        {count}
      </span>
    ...
    <button
      ...
      className={cn(
        ...
        tone === "danger"
          ...
          : "border-border/60 bg-secondary/60 text-foreground hover:border-border hover:bg-secondary",
      )}
    >
      {icon}
      {label}
...
function EmptyState({ hasAny, onClear }: { hasAny: boolean; onClear: () => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center py-16 text-center">
      <span className="grid h-14 w-14 place-items-center rounded-2xl bg-secondary/70 text-muted-foreground ring-1 ring-border/60">
      ...
      {hasAny && (
        <button
          onClick={onClear}
          className="mt-4 inline-flex h-8 items-center rounded-full border border-border/60 bg-secondary/60 px-3 text-[11.5px] font-medium text-foreground hover:bg-secondary"
        >
          Clear filters
        </button>
      )}
