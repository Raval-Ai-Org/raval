...
import {
  Bot, Plus, Trash2, Search, FileText, Share2, Pause, Play, Mail, Calendar, Repeat, Check, Wand2,
...
const PRESETS: Preset[] = [
  { label: "Daily Instagram post", taskType: "social-post", channel: "instagram", agent: "echo", cadence: "daily", hour: 9, minute: 0, icon: Share2, prompt: "Write today's Instagram post for the brand." },
  { label: "Daily LinkedIn post",  taskType: "social-post", channel: "linkedin",  agent: "echo", cadence: "daily", hour: 10, minute: 0, icon: Share2, prompt: "Write today's LinkedIn post for the brand." },
  { label: "Weekly newsletter",    taskType: "crm-message", channel: "email",     agent: "spark",cadence: "weekly", hour: 7, minute: 0, weekday: 1, icon: Mail, prompt: "Draft this week's customer newsletter." },
  { label: "Weekly SEO brief",     taskType: "seo-audit",   channel: "blog",      agent: "scout",cadence: "weekly", hour: 10, minute: 0, weekday: 2, icon: FileText, prompt: "Produce one on-page SEO brief targeting a relevant query for the brand." },
];
...
function cadenceLabel(j: Pick<ScheduledJob, "cadence" | "next_run_at">) {
  const when = new Date(j.next_run_at).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
  const c = j.cadence === "once" ? "Once" : j.cadence === "hourly" ? "Hourly" : j.cadence === "daily" ? "Daily" : "Weekly";
  return `${c} · next ${when}`;
}
...
export function ScheduleButton({ workspaceId }: { workspaceId: string | null }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setTitle(""); setDue(""); setPrompt("");
      toast.success("Scheduled", { description: new Date(iso).toLocaleString() });
      refresh();
    } catch (e) {
      toast.error("Could not schedule", { description: e instanceof Error ? e.message : String(e) });
  ...
  const del = async (id: string) => { await remove({ data: { id } }); refresh(); };
  const fireNow = async (id: string) => {
    try {
      ...
      refresh();
    } catch (e) { toast.error("Could not run", { description: e instanceof Error ? e.message : String(e) }); }
  };

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className={SEG} title="Schedule recurring posts, emails & campaigns" aria-label="Open schedule">
          <Calendar className="h-3.5 w-3.5 transition-colors group-hover:text-[hsl(var(--brand-blue))]" strokeWidth={2} aria-hidden />
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Calendar className="h-4 w-4 text-aura" /> Schedule</DialogTitle>
          <DialogDescription>Plan recurring or one-off drafts. The scheduler runs every minute — when a job is due, agents generate it grounded in your Brand DNA and add it to approvals.</DialogDescription>
        </DialogHeader>
            ...
            {PRESETS.map((p) => {
              ...
              return (
                <button key={p.label} onClick={() => addPreset(p)}
                  ...
                  <div className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-secondary text-foreground/80"><Icon className="h-3.5 w-3.5" /></div>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[12.5px] font-medium">{p.label}</div>
                    <div className="truncate text-[10.5px] text-muted-foreground">{p.cadence === "daily" ? `Daily · ${String(p.hour).padStart(2, "0")}:${String(p.minute).padStart(2, "0")}` : `Weekly · ${["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][p.weekday ?? 1]} ${p.hour}:${String(p.minute).padStart(2,"0")}`}</div>
                  </div>
        ...
        <div className="mb-3 space-y-2 rounded-xl border border-border bg-card px-2.5 py-2">
          <div className="flex items-center gap-2">
            ...
            />
            <Button size="sm" onClick={submit} disabled={!title.trim() || !due || busy || !workspaceId}>{busy ? "…" : "Schedule"}</Button>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 pl-5">
            ...
            {[
              ...
              <button key={q.l} onClick={() => quickWhen(q.m)}
        ...
        <div className="max-h-[40vh] space-y-1.5 overflow-auto pr-1">
          {jobs.length === 0 ? (
            ...
            jobs.map((t) => {
              ...
              return (
                <motion.div key={t.id} layout initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                  className={cn("group flex items-center gap-2.5 rounded-xl border border-border bg-card/60 p-2.5", !t.active && "opacity-60")}>
                  <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-secondary text-muted-foreground"><Icon className="h-3.5 w-3.5" /></div>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[13px] font-medium">{t.title}</div>
                    <div className="truncate text-[11px] text-muted-foreground">
                      {cadenceLabel(t)}
                      {t.channel ? ` · ${t.channel}` : ""}
                      {t.last_run_status === "error" ? ` · last run failed` : t.run_count > 0 ? ` · ran ${t.run_count}×` : ""}
                    </div>
                  </div>
                  <button onClick={() => fireNow(t.id)} aria-label={`Run "${t.title}" now`} className="rounded-md px-1.5 py-1 text-[10.5px] text-muted-foreground hover:bg-secondary hover:text-foreground" title="Run now">
                    <Play className="h-3 w-3" aria-hidden />
                  </button>
                  <button onClick={() => togglePause(t)} aria-label={t.active ? `Pause "${t.title}"` : `Resume "${t.title}"`} aria-pressed={!t.active} className="rounded-md px-1.5 py-1 text-[10.5px] text-muted-foreground hover:bg-secondary hover:text-foreground" title={t.active ? "Pause" : "Resume"}>
                    {t.active ? <Pause className="h-3 w-3" aria-hidden /> : <Play className="h-3 w-3" aria-hidden />}
                  </button>
                  <button onClick={() => del(t.id)} className="opacity-0 transition group-hover:opacity-100 focus-visible:opacity-100" aria-label={`Remove "${t.title}"`}>
                    <Trash2 className="h-3.5 w-3.5 text-muted-foreground hover:text-destructive" aria-hidden />
                  </button>
                </motion.div>
              );
            })
...
export function AutopilotButton() {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem("agent-cadence"); if (raw) setCadence(JSON.parse(raw)); } catch {}
  }, []);
  const updateCadence = (id: string, v: string) => {
    setCadence((c) => {
      const next = { ...c, [id]: v };
      try { localStorage.setItem("agent-cadence", JSON.stringify(next)); } catch {}
      return next;
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className={cn(PILL, anyOn && "border-emerald-500/40")} title="24/7 agents running for you" aria-label={`Autopilot — ${activeCount} of ${total} agents active`}>
          <span className="relative flex h-2 w-2" aria-hidden>
            {anyOn && <span className="absolute inset-0 animate-ping rounded-full bg-emerald-500 opacity-75" />}
            <span className={cn("relative h-2 w-2 rounded-full", anyOn ? "bg-emerald-500" : "bg-muted-foreground")} />
          </span>
          <Bot className="h-3.5 w-3.5" aria-hidden />
          <span className="hidden sm:inline tabular-nums">{activeCount}/{total} <span className="text-muted-foreground">24/7</span></span>
        </button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Bot className="h-4 w-4 text-aura" /> 24/7 Agents</DialogTitle>
          <DialogDescription>Always-on agents that watch, react and ship while you sleep. Pick a cadence for each.</DialogDescription>
        </DialogHeader>
        <div className="flex items-center justify-between rounded-xl border border-border bg-card p-3">
          <div>
            <div className="text-[13px] font-semibold">Master autopilot</div>
            <div className="text-[11.5px] text-muted-foreground">{anyOn ? "Running — pause everything." : "Paused — wake the team."}</div>
          </div>
          <Button size="sm" variant={allOn ? "outline" : "default"} onClick={() => setAll(!allOn)} className="gap-1.5">
            {allOn ? <><Pause className="h-3.5 w-3.5" /> Pause all</> : <><Play className="h-3.5 w-3.5" /> Run all</>}
          </Button>
        </div>
              ...
              <div key={a.id} className="rounded-lg border border-border bg-card/60 p-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-[11px] text-muted-foreground">{a.missions[0]?.label ?? ""}</div>
                  </div>
