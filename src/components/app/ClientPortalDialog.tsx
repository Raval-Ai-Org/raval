...
import { Users, Link as LinkIcon, Copy, Check, Inbox, Plus, Loader2, ShieldCheck, Sparkles, Calendar, FileText, Search, Trash2, Lightbulb, ThumbsUp, ThumbsDown, MessageSquare, Eye } from "@/components/ui/gemini-icons";
...
type ContentRow = {
  id: string; title: string | null; body: string | null; channel: string | null; status: string;
  kind: string; scheduled_at: string | null; hashtags: string[] | null; media_url: string | null;
  updated_at: string;
};
type ShareRow = {
  id: string; title: string; slug: string; client_name: string | null; client_email: string | null;
  allow_comments: boolean; allow_approvals: boolean; allow_download: boolean;
  expires_at: string | null; status: string; last_viewed_at: string | null; view_count: number; created_at: string;
};
type EventRow = {
  id: string; share_id: string; item_id: string | null; kind: string; body: string | null;
  actor_name: string | null; actor_email: string | null; marketer_decision: string; created_at: string;
};
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (cancel) return;
        const p = (data.events ?? []).filter((e: EventRow) =>
          e.marketer_decision === "pending" && e.kind !== "viewed",
        ).length;
        setPending(p);
      } catch {}
    };
    ...
    const id = open ? setInterval(() => { if (!document.hidden) load(); }, 60_000) : null;
    return () => { cancel = true; if (id) clearInterval(id); };
  }, [workspaceId, open]);

  const tabs = [
    { id: "inbox" as const, label: "Inbox", icon: Inbox, badge: pending, hint: "Decide on client feedback" },
    { id: "share" as const, label: "New share", icon: Plus, badge: 0, hint: "Build a review link" },
    { id: "manage" as const, label: "Manage", icon: ShieldCheck, badge: 0, hint: "Active share links" },
  ];
        ...
        <button className={PILL} title="Client portal — share for approval">
          ...
          <AnimatePresence>
            {pending > 0 && (
              ...
              >
                <span aria-hidden className="absolute inset-0 rounded-full bg-[hsl(var(--brand-green))] opacity-50 animate-ping" />
      ...
      <AppModalShell
        ...
        headerAccessory={
          <div className="hidden sm:block shrink-0">
            <StarAgent mood={pending > 0 ? "excited" : "happy"} size={56} hue={pending > 0 ? 151 : 217} />
          </div>
        }
      >
                  ...
                  {t.badge > 0 && (
                    <motion.span
                      ...
                      className={cn(
                        ...
                        active ? "bg-background/25 text-background" : "bg-[hsl(var(--brand-green))] text-background",
                      )}
                    >
                      {t.badge}
                    </motion.span>
...
function InboxView({ workspaceId }: { workspaceId: string | null }) {
  ...
  const decide = async (eventId: string, decision: "accepted" | "dismissed" | "applied") => {
    ...
    try {
      ...
      toast.success(decision === "accepted" ? "Accepted" : decision === "applied" ? "Applied" : "Dismissed");
      refresh();
    } catch (e: any) {
  ...
  const recent = events.filter((e) => e.marketer_decision !== "pending" || e.kind === "viewed").slice(0, 20);
  ...
  return (
    <div className="space-y-4">
        ...
        {pending.length === 0 ? (
          ...
          >
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-[11.5px] text-muted-foreground max-w-xs">No pending approvals or suggestions. When clients act, you'll see it land here.</div>
            </div>
          </motion.div>
        ) : (
              ...
              return (
                <motion.div
                  ...
                  <span aria-hidden className={cn(
                    ...
                    ev.kind === "approved" ? "from-emerald-400 to-emerald-600"
                    : ev.kind === "rejected" ? "from-red-400 to-red-600"
                    : ev.kind === "suggested" ? "from-[hsl(var(--brand-blue))] to-[hsl(var(--brand-green))]"
                    : "from-amber-400 to-amber-600",
                  )} />
                  <div className="flex items-start gap-3">
                    <div className={cn("mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-secondary", meta.tone)}>
                      <Icon className="h-3.5 w-3.5" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 text-[12.5px]">
                        <span className={cn("font-semibold", meta.tone)}>{meta.label}</span>
                        <span className="text-muted-foreground">by {ev.actor_name ?? "client"}</span>
                        {share && (
                          <span className="text-muted-foreground">· {share.title}</span>
                        )}
                        <span className="ml-auto text-[10.5px] text-muted-foreground">{new Date(ev.created_at).toLocaleString()}</span>
                      </div>
                      {ev.body && <p className="mt-1.5 text-[13px] whitespace-pre-wrap">{ev.body}</p>}
                      <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
                        {ev.kind === "suggested" ? (
                          <>
                            <Button size="sm" onClick={() => saveToMemory(ev)} disabled={busy === ev.id}>
                              <Lightbulb className="h-3.5 w-3.5 mr-1" /> Save to Memory
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => decide(ev.id, "dismissed")} disabled={busy === ev.id}>Dismiss</Button>
                          </>
                        ) : (
                          <>
                            <Button size="sm" onClick={() => decide(ev.id, "accepted")} disabled={busy === ev.id}>
      ...
      {recent.length > 0 && (
        <section>
          <div className="mb-2 text-[10.5px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">Activity</div>
            ...
            {recent.map((ev) => {
              ...
              return (
                <div key={ev.id} className="flex items-center gap-2.5 px-3 py-2 text-[12px]">
                  ...
                  <span className="ml-auto text-[10.5px] text-muted-foreground shrink-0">{new Date(ev.created_at).toLocaleString()}</span>
                </div>
              );
            })}
          </div>
...
/* ───────── New share ───────── */
function NewShareView({ workspaceId, onCreated }: { workspaceId: string | null; onCreated: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from("content_items")
      .select("id, title, body, channel, status, kind, scheduled_at, hashtags, media_url, updated_at")
      .eq("workspace_id", workspaceId)
      .order("updated_at", { ascending: false })
      .limit(50)
      .then(({ data }) => { setContent((data ?? []) as any); setLoadingContent(false); });
  }, [workspaceId]);
  ...
  const create = async () => {
    ...
    const items = content.filter((c) => picked[c.id]).map((c) => ({
      kind: "content_item" as const,
      refId: c.id,
      title: c.title ?? "Untitled",
      description: c.body?.slice(0, 200) ?? "",
      snapshot: {
        body: c.body, channel: c.channel, kind: c.kind,
        scheduled_at: c.scheduled_at, hashtags: c.hashtags, media_url: c.media_url,
      },
    }));
    if (items.length === 0) { toast.error("Pick at least one item to share"); return; }
    ...
    try {
      ...
      const r = await authedFetch("/api/shares?action=create", {
        ...
        body: JSON.stringify({
          ...
          expiresAt,
          allowComments, allowApprovals, allowDownload,
          items,
        }),
      });
      if (!r.ok) throw new Error(await r.text());
      ...
      try { await navigator.clipboard.writeText(data.url); setCopied(true); } catch {}
      ...
      <div className="space-y-4">
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5 text-center">
          ...
          <div className="mt-3 flex items-center gap-1.5 rounded-lg border border-border/60 bg-background px-2.5 py-1.5">
            ...
            <input readOnly value={created.url} className="flex-1 truncate bg-transparent text-[12px] outline-none" />
          ...
          <div className="mt-3 flex items-center justify-center gap-2">
            <Button size="sm" variant="ghost" onClick={() => window.open(created.url, "_blank")}>Preview as client</Button>
  ...
  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-2">
        <div>
          <Label>Title</Label>
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Acme — week of June 30" />
        </div>
        <div>
        ...
        <div>
          ...
          <Input value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} placeholder="hello@acme.com" />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          ...
          <Input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Leave blank for link-only access" type="text" />
        </div>
      </div>
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <Label>What to share <span className="text-muted-foreground">({selectedCount} picked)</span></Label>
          <button onClick={() => setPicked(Object.fromEntries(content.slice(0, 10).map((c) => [c.id, true])))}
            className="text-[11px] text-muted-foreground hover:text-foreground">Pick latest 10</button>
        </div>
        {loadingContent ? (
          ...
          <div className="max-h-[300px] overflow-y-auto rounded-xl border border-border/60 divide-y divide-border/40">
            {content.map((c) => {
              const Icon = c.kind === "post" ? Sparkles : c.kind === "blog" ? FileText : c.kind === "brief" ? Search : c.kind === "email" ? FileText : Calendar;
              return (
                <label key={c.id} className="flex items-center gap-2.5 px-3 py-2 hover:bg-secondary/40 cursor-pointer">
                  <input type="checkbox" checked={!!picked[c.id]}
                    onChange={(e) => setPicked((p) => ({ ...p, [c.id]: e.target.checked }))} />
                  <Icon className="h-3.5 w-3.5 text-muted-foreground" />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[12.5px] font-medium">{c.title ?? c.body?.slice(0, 60) ?? "Untitled"}</div>
...
function ManageView({ workspaceId }: { workspaceId: string | null }) {
  ...
  const copyLink = async (s: ShareRow) => {
    ...
    try { await navigator.clipboard.writeText(url); setCopiedId(s.id); setTimeout(() => setCopiedId(null), 1500); } catch {}
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="text-[11.5px] text-muted-foreground max-w-xs">Switch to <span className="text-foreground font-medium">New share</span> to build a branded review link for your client.</div>
        </div>
      </motion.div>
    );
  }
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 text-[13px] font-semibold">
                {s.title}
                {s.status !== "active" && <span className="text-[10px] uppercase tracking-wide text-red-600">Revoked</span>}
              </div>
            ...
            <div className="flex items-center gap-1">
              ...
              {s.status === "active" && (
                <Button size="sm" variant="ghost" onClick={() => revoke(s.id)} disabled={busy === s.id} title="Revoke share">
...
function Label({ children }: { children: React.ReactNode }) {
  return <div className="mb-1 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">{children}</div>;
}
  ...
  return (
    <button onClick={() => onChange(!value)}
      className={cn(
        ...
        value ? "border-foreground/20 bg-card" : "border-border/60 bg-background text-muted-foreground",
      )}>
      <span>{label}</span>
...
function Badge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-border/60 bg-background px-1.5 py-px text-[9.5px] font-medium uppercase tracking-wide">{children}</span>;
}
