...
import {
  LayoutGrid, Search, FileText, Share2, BarChart3, Users,
  Power, Sparkles, ArrowRight, MessageSquare, Brain, CheckSquare, Bot,
  Zap, CornerDownLeft,
...
const ROUTES = [
  { to: "/app",            label: "Chat",                 icon: LayoutGrid, search: undefined as any, analyticsTab: undefined as string | undefined },
  { to: "/app",            label: "Analytics · Overview", icon: BarChart3,  search: undefined,        analyticsTab: "overview" },
  { to: "/app",            label: "Analytics · Organic",  icon: Search,     search: undefined,        analyticsTab: "organic"  },
  { to: "/app",            label: "Analytics · Social",   icon: Share2,     search: undefined,        analyticsTab: "social"   },
  { to: "/app",            label: "Analytics · Content",  icon: FileText,   search: undefined,        analyticsTab: "content"  },
  { to: "/app",            label: "Analytics · Automations", icon: Bot,     search: undefined,        analyticsTab: "automations" },
] as const;
...
export function CommandBar() {
  ...
  const toggle = (id: string, on: boolean) => { set(id, on); setOpen(false); };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ boxShadow: "0 30px 80px -20px hsl(217 91% 50% / 0.35), 0 1px 0 0 hsl(var(--border))" }}
      >
        <Command label="Command Menu" className="flex flex-col" shouldFilter={true}>
          <div className="flex items-center gap-2 border-b border-border bg-gradient-to-b from-card to-popover px-3.5">
            ...
            />
            <kbd className="hidden sm:inline rounded border border-border bg-background px-1.5 py-0.5 text-[10px] text-muted-foreground">ESC</kbd>
          </div>
          <Command.List className="max-h-[60vh] overflow-y-auto p-1.5 scrollbar-thin">
            ...
            {trimmed.length > 0 && (
              ...
              >
                ...
                <span className="flex-1 min-w-0">
                  <span className="block text-[10.5px] font-semibold uppercase tracking-wider text-primary">Ask Raval Ai</span>
                  <span className="block truncate text-[13px] font-medium text-foreground">{trimmed}</span>
                </span>
            ...
            <Command.Empty className="px-3 py-6 text-center text-sm text-muted-foreground">
              No matches. Press <kbd className="rounded border border-border bg-background px-1 py-0.5 text-[10px]">↵</kbd> to send "{trimmed}" to chat.
            </Command.Empty>

            {trimmed.length === 0 && (
              <Command.Group heading="Suggested prompts" className="px-1 pt-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                {QUICK_PROMPTS.map((p) => (
                  <Command.Item
                    key={p.label}
                    value={`ask ${p.label}`}
            ...
            <Command.Group heading="Navigate" className="px-1 pt-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {ROUTES.map((r) => (
                <Command.Item
                  key={r.label}
                  value={`go ${r.label}`}
                  onSelect={() => {
                    if (r.analyticsTab) {
                      setOpen(false);
                      window.dispatchEvent(new CustomEvent("open:analytics", { detail: { tab: r.analyticsTab } }));
            ...
            <Command.Group heading="Studio · create" className="px-1 pt-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {STUDIO_TILES.map((t) => (
                <Command.Item
                  key={t.id}
                  ...
                  onSelect={() => {
                    setOpen(false);
                    window.dispatchEvent(new CustomEvent("open:canvas", { detail: { type: t.id } }));
                ...
                >
                  ...
                  <span className="flex-1">Create {t.label}<span className="ml-1.5 text-xs text-muted-foreground">{t.sub}</span></span>
                  <ArrowRight className="h-3.5 w-3.5 text-muted-foreground" />
                </Command.Item>
              ))}
            </Command.Group>

            <Command.Group heading="Workspace" className="px-1 pt-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
            ...
            <Command.Group heading="Agents" className="px-1 pt-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {agentList.map((a) => {
                const on = isOn(a.id);
                return (
                  <Command.Item
              ...
              <Command.Item value="all agents on" onSelect={() => { setAll(true); setOpen(false); }}
                className="flex cursor-pointer items-center gap-2.5 rounded-md px-2.5 py-2 text-sm data-[selected=true]:bg-secondary">
                <Power className="h-4 w-4 text-success" />
                <span>Activate all agents</span>
              </Command.Item>
              <Command.Item value="all agents off pause" onSelect={() => { setAll(false); setOpen(false); }}
                className="flex cursor-pointer items-center gap-2.5 rounded-md px-2.5 py-2 text-sm data-[selected=true]:bg-secondary">
          ...
          <div className="flex items-center justify-between border-t border-border bg-card/50 px-3 py-2 text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1.5"><Sparkles className="h-3 w-3 text-aura" /> Raval Ai · Universal command</span>
            <span className="flex items-center gap-1">
              <kbd className="rounded border border-border bg-background px-1 py-0.5">↑↓</kbd> nav
              <kbd className="ml-1 rounded border border-border bg-background px-1 py-0.5">↵</kbd> run
              <kbd className="ml-1 rounded border border-border bg-background px-1 py-0.5">⌘K</kbd> close
            </span>
          </div>
        </Command>
      </div>
