...
import {
  ArrowUp, Sparkles, ChevronDown, Square, Search, Check,
  Paperclip, SlidersHorizontal, Target, Globe, MessageSquare, Bot, Command as CmdIcon,
  Sun, Moon,
} from "@/components/brand/icons";
import { Rows3, Rows2, Zap, ZapOff, FileText, FileSpreadsheet, FileImage, File as FileIcon, X, Loader2, Plus } from "@/components/ui/gemini-icons";
...
import { Zap as ZapIcon, Wand2, BookOpen, CalendarDays, ArrowUpRight, CheckCircle2, AlertTriangle } from "@/components/ui/gemini-icons";
...
const SUGGESTIONS: { label: string; icon: any; hint: string }[] = [
  ...
  { label: "Find Reddit threads to reply to",     icon: MessageSquare, hint: "Surface high-intent threads" },
...
export function ChatPanel({ workspaceId, variant = "rail", mobileAccessory }: { workspaceId: string; variant?: "rail" | "centered"; mobileAccessory?: ReactNode }) {
  ...
  const [wsStats, setWsStats] = useState<{ pending: number; scheduled: number; published: number; recentTitles: string[] } | null>(null);
  ...
  const addFiles = async (fileList: FileList | File[]) => {
    ...
    for (const att of staged) {
      try {
        ...
        setAttachments((prev) => prev.map((x) => x.id === att.id ? { ...x, status: "ready", text, preview } : x));
      } catch (e: any) {
        setAttachments((prev) => prev.map((x) => x.id === att.id ? { ...x, status: "error", error: e?.message ?? "Failed to read" } : x));
  ...
  const mFade = useMemo(
    () => (reducedMotion
      ...
      : { initial: { opacity: 0, y: 8 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.22, ease: [0.22, 1, 0.36, 1] as any } }),
    [reducedMotion],
  );
  ...
  useEffect(() => {
    ...
    supabase.from("chat_messages").select("*").eq("workspace_id", workspaceId)
      .order("created_at", { ascending: true }).limit(50)
      .then(({ data }) => {
        if (data) setMessages(data as any);
        setMessagesLoaded(true);
      });
    supabase.from("workspaces").select("website_url, first_prompt").eq("id", workspaceId).maybeSingle()
      .then(({ data }) => {
        ...
        let locked = false;
        try { locked = typeof window !== "undefined" && window.localStorage.getItem(firstPromptLockKey) === "1"; } catch { /* noop */ }
    ...
    supabase.from("content_items")
      ...
      .then(({ data }) => {
        if (!data) return;
        const pending = data.filter((r: any) => r.status === "pending" || r.status === "needs_approval").length;
        const scheduled = data.filter((r: any) => r.status === "scheduled").length;
        const published = data.filter((r: any) => r.status === "published").length;
        const recentTitles = data.slice(0, 6).map((r: any) => `${r.channel ?? "post"}: ${r.title ?? "(untitled)"} [${r.status}]`);
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(b?.todaysMoves)) bits.push(`Today: ${b.todaysMoves.slice(0, 3).map((m: any) => m.title || m).join(" · ")}`);
        if (Array.isArray(b?.marketSignals)) bits.push(`Market: ${b.marketSignals.slice(0, 2).map((m: any) => m.title || m.signal || m).join(" · ")}`);
  ...
  const ctxSources = useMemo(() => {
    ...
    push("brand-extended", "Brand context", brandExtended.join("\n"),
      ["brand","positioning","mission","values","story","usp","identity","industry","about"]);

    // Products
    if (dna.products) push("products", "Products", dna.products,
      ["product","products","feature","features","offering","service","services","pricing","sku"]);
    ...
    push("rules", "Voice rules", rules.join("\n"),
      ["write","post","copy","draft","tone","voice","email","caption","headline","hook","cta"],
      { baseScore: 1 });

    // Visual identity
    const visual: string[] = [];
    if (dna.colors?.length) visual.push(`Colors: ${dna.colors.slice(0, 4).map((c) => `${c.name} ${c.hex}`).join(", ")}`);
    if (dna.fonts?.length) visual.push(`Fonts: ${dna.fonts.slice(0, 3).join(", ")}`);
    push("visual", "Visual identity", visual.join("\n"),
      ["image","logo","color","colors","font","fonts","design","visual","brand kit","palette","typography"]);

    // Persistent user insights — high value; keep baseScore so they always compete
    if (dna.userInsights?.length) {
      const body = dna.userInsights.slice(0, 10)
        .map((n) => `- ${n.title}: ${n.body}`).join("\n");
      push("insights", "Operator insights (respect these)", body,
    ...
    if (dna.competitors?.length) {
      const body = dna.competitors.slice(0, 6)
        .map((c) => `- ${c.name}${c.positioning ? ` — ${c.positioning}` : ""}`).join("\n");
      push("competitors", "Competitors", body,
        ["competitor","competitors","rival","market","landscape","versus","vs","compare"]);
    }
    ...
    if (cs && (cs.triggerSignals?.length || cs.objectionSignals?.length || cs.personas?.length)) {
      const parts: string[] = [];
      if (cs.personas?.length) parts.push(`Personas: ${cs.personas.slice(0, 4).map((p) => p.name).join(", ")}`);
      if (cs.triggerSignals?.length) parts.push(`Triggers: ${cs.triggerSignals.slice(0, 4).map((s) => s.text).join(" | ")}`);
      if (cs.objectionSignals?.length) parts.push(`Objections: ${cs.objectionSignals.slice(0, 4).map((s) => s.text).join(" | ")}`);
      push("customer", "Customer signals", parts.join("\n"),
        ["customer","persona","audience","buyer","objection","trigger","pain","icp","segment"]);
    }

    // Workspace activity
    if (wsStats && (wsStats.pending || wsStats.scheduled || wsStats.published || wsStats.recentTitles.length)) {
      const parts: string[] = [];
      ...
      if (wsStats.recentTitles.length) parts.push(`Recent: ${wsStats.recentTitles.slice(0, 4).join(" • ")}`);
      push("workspace", "Workspace activity", parts.join("\n"),
        ["approve","approval","schedule","scheduled","publish","published","calendar","pending","status","queue","recent"]);
    }

    // Coach briefing
    if (coachSummary) push("coach", "Coach briefing", coachSummary,
      ["coach","strategy","plan","priority","focus","week","goal","brief"],
      { maxChars: 500 });

    // Competitor alerts
    if (competitorSummary) push("comp-alerts", "Competitor alerts", competitorSummary,
  ...
  const runChatStream = async (history: { role: string; content: string }[]) => {
    setStreaming(true);
    window.dispatchEvent(new CustomEvent("chat:working", { detail: { label: "Agents working on your site…" } }));
    ...
    try {
      ...
      const compactMessages = compactHistory(history as import("@/lib/ai/history-compact").ChatTurn[]);
      ...
      try {
        ...
        if (calls.length) {
          const saveMemory = async (title: string, body: string) => {
            const note = { id: crypto.randomUUID(), title, body, createdAt: Date.now(), source: "chat" as const };
            const current = dnaRef.current;
          ...
          setMessages((m) => [...m, {
            id: crypto.randomUUID(), role: "assistant", kind: "actions", content: "",
            payload: { results },
          }]);
        }
      } catch (e) { console.warn("tool parse failed", e); }

      await supabase.from("chat_messages").insert({ workspace_id: workspaceId, role: "assistant", kind: "text", content: acc });
    } catch {
  ...
  const continueAfterClarify = async (clarifyMsgId: string, answers: Record<string, string[]>, payload: ClarifyPayload) => {
    // Render the answers summary into the clarify card and lock it.
    setMessages((m) => m.map((x) => x.id === clarifyMsgId ? { ...x, payload: { ...x.payload, done: true, submitted: answers } } : x));
    setClarifying(false);
    ...
    const synthetic: Msg = { id: crypto.randomUUID(), role: "user", kind: "text", content: userClarification };
    setMessages((m) => [...m, synthetic]);
    await supabase.from("chat_messages").insert({ workspace_id: workspaceId, role: "user", kind: "text", content: userClarification });
  ...
  const send = async (override?: string) => {
    ...
    const visibleText = text || (hasAttachments ? `Please analyze the attached ${attachments.length === 1 ? "file" : "files"}.` : "");
    ...
    const userMsg: Msg = { id: crypto.randomUUID(), role: "user", kind: "text", content: displayContent };
    ...
    await supabase.from("chat_messages").insert({ workspace_id: workspaceId, role: "user", kind: "text", content: displayContent });
    ...
    if (actions.length) {
      setMessages((m) => [...m, { id: crypto.randomUUID(), role: "assistant", kind: "actions", content: "", payload: { actions } }]);
    }
    ...
    if (payload) {
      ...
      setMessages((m) => [...m, { id: cId, role: "assistant", kind: "clarify", content: "", payload: { ...payload, done: false } }]);
    ...
    setClarifying(false);
    const history = [...messages, userMsg].slice(-12).map((m) => ({ role: m.role, content: (m as any)._wire || m.content }));
    await runChatStream(history);
  };
  ...
  const runAutoSendOnce = async (prompt: string): Promise<boolean> => {
    ...
    const timeoutP = new Promise<void>((resolve) => {
      window.setTimeout(() => { timedOut = true; resolve(); }, AUTO_SEND_TIMEOUT_MS);
    });
  ...
  const runAutoSendWithRetries = async (prompt: string) => {
    ...
    for (let attempt = 1; attempt <= AUTO_SEND_MAX_RETRIES + 1; attempt++) {
      ...
      if (attempt <= AUTO_SEND_MAX_RETRIES) {
        ...
        setMessages((m) => {
          const idx = [...m].reverse().findIndex((x) => x.role === "assistant" && x.kind === "text" && !x.content?.trim());
          if (idx === -1) return m;
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) {
      ...
      try { window.localStorage.setItem(firstPromptLockKey, "1"); } catch { /* noop */ }
      void supabase.from("workspaces").update({ first_prompt: null }).eq("id", workspaceId);
      setPendingFirstPrompt(null);
      return;
    }
  ...
  }, [pendingFirstPrompt, messagesLoaded, messages.length, workspaceId, streaming, clarifying, firstPromptLockKey]);
  ...
  return (
    <div className={centered ? "flex h-full flex-col bg-transparent" : "flex h-full flex-col bg-gradient-to-b from-background via-background to-background/60"}>
        ...
        {autoSending && empty ? (
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            >
              ...
              <div className="relative grid h-14 w-14 place-items-center rounded-2xl border border-border/70 bg-card/70 backdrop-blur">
                <Loader2 className="h-6 w-6 animate-spin text-[hsl(var(--brand-green))]" aria-hidden />
              </div>
            </motion.div>
            <h2 className="text-[18px] font-semibold tracking-tight text-foreground">
              {autoSendAttempt > 1 ? `Retrying your first request… (attempt ${autoSendAttempt})` : "Kicking off your first request…"}
            </h2>
            <p className="mt-2 max-w-xs text-[13px] leading-relaxed text-muted-foreground">
              Raval AI is reading your brand context and preparing an answer. This takes a few seconds.
            </p>
            <div className="mt-5 flex items-center gap-1.5" aria-hidden>
              <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[hsl(var(--brand-green))]" style={{ animationDelay: "0ms" }} />
              <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[hsl(var(--brand-green))]" style={{ animationDelay: "150ms" }} />
              <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[hsl(var(--brand-green))]" style={{ animationDelay: "300ms" }} />
            </div>
          </div>
          ...
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <div className="mt-2 max-w-xs rounded-lg border border-border/60 bg-muted/40 px-3 py-2 text-left text-[12px] text-muted-foreground">
              <span className="font-medium text-foreground">Your prompt:</span> {autoSendPromptRef.current}
            </div>
            <button
          ...
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <h2 className="text-[20px] font-semibold tracking-tight text-foreground">How can I help you grow?</h2>
            <p className="mt-2 max-w-xs text-[13px] leading-relaxed text-muted-foreground">
              Ask anything about your site — SEO, content, social, ads. Work shows up live on the right.
            </p>
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === "ArrowDown" || e.key === "ArrowRight") {
                  ...
                  e.preventDefault();
                  setSuggestionFocus(0); suggestionRefs.current[0]?.focus();
                } else if (e.key === "End") {
                  e.preventDefault();
                  const last = SUGGESTIONS.length - 1;
                  setSuggestionFocus(last); suggestionRefs.current[last]?.focus();
                }
              }}
            >
              {SUGGESTIONS.map((s, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 min-w-0">
                      <span className="block truncate font-sans text-[12.5px] font-medium text-foreground">{s.label}</span>
                      <span className="block truncate font-sans text-[11px] text-muted-foreground">{s.hint}</span>
                    </span>
                    <ArrowUp className="h-3 w-3 rotate-45 text-muted-foreground opacity-0 transition group-hover:opacity-100" aria-hidden />
                  </motion.button>
                );
              })}
            </div>
          ...
          <div className="space-y-5 px-4 py-5 md:space-y-6 md:px-5 md:py-6">
                  ...
                  return (
                    ...
                    >
                      ...
                      <ClarifyCard
                        ...
                        onSkip={() => {
                          setMessages((arr) => arr.map((x) => x.id === m.id ? { ...x, payload: { ...x.payload, done: true, submitted: {} } } : x));
                          setClarifying(false);
            ...
            {clarifying && !messages.some((m) => m.kind === "clarify" && !m.payload?.done) && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="ml-10 flex items-center gap-2 text-[11.5px]">
      ...
      <div className={centered ? `shrink-0 px-3 pb-[max(0.875rem,env(safe-area-inset-bottom))] ${mobileAccessory ? "pt-1" : "pt-3"} md:px-4` : "shrink-0 px-3 pb-[max(0.875rem,env(safe-area-inset-bottom))] pt-3 md:px-4"}>
        {mobileAccessory && (
        ...
        <div
          ...
          onDrop={onDrop}
        >
        <div className={`prompt-pill px-2.5 py-2 transition ${dragging ? "ring-2 ring-primary/60 ring-offset-2 ring-offset-background" : ""}`}>
          ...
          {attachments.length > 0 && (
            <div className="mb-2 flex flex-wrap gap-1.5 px-2 pt-1">
              {attachments.map((a) => {
                const Icon = a.kind === "image" ? FileImage
                  : a.kind === "xlsx" ? FileSpreadsheet
                  : a.kind === "pdf" || a.kind === "docx" || a.kind === "text" ? FileText
                  : FileIcon;
                const tint =
                  a.kind === "image" ? "text-fuchsia-400"
                  : a.kind === "xlsx" ? "text-emerald-400"
                  : a.kind === "pdf" ? "text-rose-400"
                  : a.kind === "docx" ? "text-sky-400"
                ...
                return (
                  ...
                  >
                    {a.preview ? (
                      ...
                      <span className={`grid h-7 w-7 place-items-center rounded-md bg-secondary ${tint}`}>
                        {a.status === "reading"
          ...
          <div className="flex items-center gap-2">
            ...
            <textarea
              ...
              onKeyDown={(e) => {
                ...
                if (e.key === "Enter" && !e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey && !composing) {
                  e.preventDefault();
                  if (!autoSending) send();
                }
              }}
            ...
            <button
              onClick={() => { if (!autoSending) send(); }}
              disabled={autoSending || ((!input.trim() && !attachments.length) && !streaming)}
              aria-label={autoSending ? "Sending onboarding prompt" : streaming ? "Stop generating" : "Send message"}
              aria-busy={autoSending}
