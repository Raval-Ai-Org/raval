...
export function useStudioSuggestions() {
  ...
  const load = useCallback(async () => {
    ...
    if ((publishedRecent.count ?? 0) < 3) {
      out.push({
        ...
        accent: "green",
        icon: "Calendar",
        run: () => chatPrefill("Plan this week's content — 5 posts across LinkedIn and Instagram, grounded in our Brand DNA."),
      });
    }
    ...
    if ((scheduledNext.count ?? 0) === 0) {
      out.push({
        ...
        accent: "indigo",
        icon: "Calendar",
        run: () => chatPrefill("Schedule 5 posts for next week across LinkedIn and Instagram with the best times for our audience."),
      });
    }
    ...
    if (out.length === 0) {
      out.push({
        ...
        accent: "indigo",
        icon: "Sparkles",
        run: () => chatPrefill("Brainstorm 5 campaign angles for our next launch, grounded in our Brand DNA."),
      });
    }
      ...
      const brandContext = (() => {
        try {
          for (const k of [`brand-dna:v3:${wsId}`, `brand-dna:v2:${wsId}`, `brand-dna:${wsId}`]) {
            ...
            const b = JSON.parse(raw) as Record<string, unknown>;
            const lines: string[] = [];
            for (const f of ["brandName", "oneLiner", "industry", "products", "audience", "voice", "values"]) {
              const v = b[f];
  ...
  useEffect(() => {
    ...
    const onSignalChange = () => { loadAi(true).catch(() => {}); };
    ...
    const t = window.setInterval(() => { if (!document.hidden) load().catch(() => {}); }, 120000);
    const onVis = () => { if (!document.hidden) load().catch(() => {}); };
  ...
  const merged = [...items, ...aiItems.filter((a) => !items.some((i) => i.label === a.label))].slice(0, 8);
...
function aiAccentFor(intent: string): StudioSuggestionAccent {
  switch (intent) {
    case "geo-audit": return "blue";
    case "brand-dna": return "violet";
    case "plan-week":
    case "schedule": return "indigo";
    case "review-drafts": return "amber";
    case "seo-brief":
    case "blog": return "blue";
    case "share": return "rose";
...
function aiIconFor(intent: string): StudioSuggestion["icon"] {
  switch (intent) {
    case "geo-audit": return "Search";
    case "brand-dna": return "Brain";
    case "plan-week":
    case "schedule": return "Calendar";
    case "review-drafts": return "Wand2";
    case "seo-brief":
