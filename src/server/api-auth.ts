...
export type WorkspaceAccess =
  (VerifiedUser & { workspaceId: string }) | { ok: false; response: Response };

/**
 * Confirm an authenticated user belongs to `workspaceId` (400 missing or
 * malformed id, 403 not a member). Uses the caller's RLS-bound client.
 */
...
export async function requireWorkspaceAccess(
  ...
  const membership = await checkWorkspaceMembership(auth, workspaceId);
  if (!membership.ok) return membership;
  return { ...auth, workspaceId: membership.workspaceId };
}
