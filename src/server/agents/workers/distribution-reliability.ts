...
export function analyzeDeliveryEvidence(ev: DeliveryEvidence): Finding[] {
  ...
  if (ev.stale.length) {
    ...
    findings.push({
      ...
      evidence: ev.stale
        .slice(0, 10)
        .map((r) => `${r.platform} delivery ${r.id} (${r.status}, last update ${r.updated_at})`),
      hypotheses: [
        reconcileOverdue
  ...
  for (const [account, rows] of authByAccount) {
    findings.push({
      ...
      evidence: rows
        .slice(0, 10)
        .map((r) => `${r.platform} delivery ${r.id}: ${r.last_error ?? "auth failure"}`),
  ...
  for (const [key, rows] of other) {
    ...
    findings.push({
      ...
      affected: rows.slice(0, 25).map((r) => ({ table: "content_publications", id: r.id })),
      recommendedAction:
        "Review the failed items; fix the content or wait out the limit, then re-publish.",
      requiresHumanApproval: true,
      confidence: 0.6,
    });
  }
  ...
  if (ev.contradictions.length) {
    findings.push({
      ...
      summary:
        "Editorial status and delivery rows disagree; the calendar may be showing the wrong state.",
      evidence: ev.contradictions
        .slice(0, 10)
        .map(
          (c) =>
            `item ${c.contentItemId} is "${c.itemStatus}" but deliveries are [${c.deliveryStatuses.join(", ")}]`,
        ),
      hypotheses: [
        "A webhook was applied without recomputing the item status, or a status was edited by hand.",
      ],
      affected: ev.contradictions
        .slice(0, 25)
        .map((c) => ({ table: "content_items", id: c.contentItemId })),
  ...
  if (bad >= 3) {
    findings.push({
      ...
      affected: [],
      recommendedAction:
        "Compare the workspace webhook secret on both services; check server clocks. Rejected callbacks changed nothing.",
      requiresHumanApproval: true,
      confidence: 0.75,
    });
  }
  ...
  if (overdue.length) {
    findings.push({
      fingerprint: `scheduler-overdue:${overdue
        .map((h) => h.job)
        .sort()
        .join(",")}`,
      ...
      hypotheses: [
        "pg_cron jobs are not scheduled, the Vault secrets are missing, or the app rejected the cron secret.",
      ],
      affected: [],
      recommendedAction:
        "Follow the scheduler section of the operations runbook (docs/OPERATIONS-RUNBOOK.md).",
      requiresHumanApproval: true,
      confidence: 0.85,
    });
  }
    ...
    if (findings.length && ctx.input.explain !== false) {
      try {
        ...
        for (const e of out.explanations) {
          ...
          if (f && e.hypothesis.trim())
            f.hypotheses = [e.hypothesis.trim(), ...f.hypotheses].slice(0, 4);
        }
        await ctx.note("Model added plain-language hypotheses", {
          explained: out.explanations.length,
        });
      } catch (error) {
