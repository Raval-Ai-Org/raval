...
if [ ! -f .env ]; then
  ...
  AUTH_PLACEHOLDER_COUNT=$(grep -cE "YOUR_PROJECT_REF|YOUR_PUBLISHABLE|YOUR_SERVICE_ROLE" .env 2>/dev/null | tr -d '[:space:]' || echo 0)
  if [ -n "$AUTH_PLACEHOLDER_COUNT" ] && [ "$AUTH_PLACEHOLDER_COUNT" -gt 0 ] 2>/dev/null; then
    fail ".env contains $AUTH_PLACEHOLDER_COUNT Supabase placeholder value(s) — auth will fail"
  ...
  AI_PLACEHOLDER_COUNT=$(grep -cE "your-openrouter|sk-or-v1-replace-me-later" .env 2>/dev/null | tr -d '[:space:]' || echo 0)
  if [ -n "$AI_PLACEHOLDER_COUNT" ] && [ "$AI_PLACEHOLDER_COUNT" -gt 0 ] 2>/dev/null; then
...
if [ -f .env ]; then
  ...
  if [ -n "$SDR_URL" ] && [ "$SDR_URL" != "https://YOUR_PROJECT_REF.supabase.co" ]; then
    ...
    HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 "$SDR_URL/healthz" 2>/dev/null | tr -d '[:space:]' || echo "000")
