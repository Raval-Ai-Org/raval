...
async function main() {
  ...
  for (const { name } of tables) {
    ...
    for (const c of cols) {
      push(
        `          ${key(c.column_name)}: ${tsType(c.udt_name, enums)}${c.nullable ? " | null" : ""};`,
      );
    }
    push("        };");
    push("        Insert: {");
  ...
  for (const [name, labels] of enums)
    push(`      ${key(name)}: ${labels.map((l) => JSON.stringify(l)).join(" | ")};`);
  push("    };");
  push("    CompositeTypes: {");
  push("      [_ in never]: never;");
  push("    };");
  ...
  console.log(
    `✓ wrote ${path.relative(REPO_ROOT, OUT)} (${tables.length} tables, ${seen.size} functions, ${enums.size} enums)`,
  );
  await db.close();
}
