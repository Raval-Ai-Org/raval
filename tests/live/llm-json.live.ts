// Live check of the structured premium-tier routes (Claude Opus 5.5 through
// OpenRouter): Brand DNA synthesis, a coach-style briefing and a real
// market-intelligence analysis. Each must finish in ONE metered call without
// truncation. Opt-in (real spend, roughly $0.05-0.15):
//   npx vitest run --config vitest.live.config.ts tests/live/llm-json.live.ts
...
describeLive("Structured output on the premium tier (live)", () => {
  ...
  it("returns a complete coach briefing from one schema-constrained call", async () => {
    ...
    const briefing = await llmJson<Record<string, unknown>>({
