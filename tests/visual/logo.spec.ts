...
/**
 * Visual regression guard for the Raval AI brand lockup (<Logo />).
 *
 ...
 *      (mark height ≈ 70% of lockup height by design).
...
//   desktop = heightProp * 0.75      // default heightProp = 18  → 13.5px
...
test.describe("Logo — size regression", () => {
  test("landing header logo fits its clamp envelope", async ({ page, viewport }) => {
    await page.goto("/");
    const logo = page.getByRole("img", { name: "Raval AI" }).first();
    ...
    // Mark : lockup ratio (mark should be ~70% of overall height).
