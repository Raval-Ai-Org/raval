...
describe("publish media pre-flight (FR-019/FR-020)", () => {
  ...
  beforeAll(async () => {
    await sdr.start();
    sdr.addAccount({ account_id: "ig-1", platform: "instagram", platform_username: "Brand IG", status: "active" });
  });
  ...
  it("text-only Instagram publish is rejected 422 (requires exactly one image)", async () => {
    const out = await publishContentItemsHandler(
      ...
      deps([{ id: "ig-item", workspace_id: "ws-1", body: "caption", media_url: null, status: "approved", meta: { platform: "instagram" } }]),
    );
    expect(out.status).toBe(422);
  });

  it("Instagram with one image publishes and passes the durable media URL to the SDR", async () => {
    sdr.reset(); sdr.addAccount({ account_id: "ig-1", platform: "instagram", platform_username: "Brand IG", status: "active" });
    const out = await publishContentItemsHandler(
      { workspaceId: "ws-1", contentItemIds: ["ig-item"], selection: { type: "all" } },
      deps([{ id: "ig-item", workspace_id: "ws-1", body: "caption", media_url: "https://cdn.example.com/image.jpg", status: "approved", meta: { platform: "instagram" } }]),
    );
