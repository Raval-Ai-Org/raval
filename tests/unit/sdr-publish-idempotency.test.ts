...
describe("publish idempotency (FR-006/SC-003)", () => {
  ...
  it("re-submitting the same action returns the existing job and does not duplicate", async () => {
    ...
    const publishReqs = sdr
      .getRequests()
      .filter((r) => r.path === "/api/v1/publish" && r.method === "POST");
  ...
  it("a different destination selection is a different job (not suppressed)", async () => {
    ...
    const single = await publishContentItemsHandler(
      {
        workspaceId: "ws-1",
    ...
    const all = await publishContentItemsHandler(
      { workspaceId: "ws-1", contentItemIds: ["item-1"], selection: { type: "all" } },
      deps(),
    );
