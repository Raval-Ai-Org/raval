...
describe("handleSdrDisabled (degraded mock — US5)", () => {
  it("publish degrades to a server-side status flip (published), no SDR involved", async () => {
    const db = makeMockDb({ content_items: [{ id: "item-1", workspace_id: "ws-1", status: "draft" }] });
    const out = await handleSdrDisabled({ workspaceId: "ws-1", contentItemIds: ["item-1"], kind: "publish" }, { db });
  ...
  it("schedule degrades to scheduled at the requested time", async () => {
    ...
    const out = await handleSdrDisabled({ workspaceId: "ws-1", contentItemIds: ["item-1"], kind: "schedule", scheduledAt: at }, { db });
