...
describe("cancelScheduledHandler", () => {
  ...
  const deps = (dbItems: MockContentItem[]): PublishDeps => ({ sdrBaseUrl: sdr.baseUrl, token: "ws-key", db: makeMockContentDb(dbItems) });
  ...
  it("cancels a pending schedule (204): SDR job cancelled, publications cancelled, item actionable", async () => {
    ...
    sdr.addJob({
      job_id: "job-scheduled-1", workspace_id: "ws-1", idempotency_key: "schedule:x", status: "pending",
      targets: [{ target_id: "t1", account_id: "tw-1", platform: "twitter", status: "pending" }],
    });
    const out = await cancelScheduledHandler({ workspaceId: "ws-1", contentItemId: "item-1" }, { sdrBaseUrl: sdr.baseUrl, token: "ws-key", db });
  ...
  it("returns 400 when the schedule already fired (SDR rejects cancel of published/failed)", async () => {
    sdr.addJob({
      job_id: "job-fired-1", workspace_id: "ws-1", idempotency_key: "schedule:y", status: "published",
      targets: [{ target_id: "t2", account_id: "tw-1", platform: "twitter", status: "published" }],
    });
    const fired: MockContentItem = { ...scheduledItem, meta: { platform: "twitter", sdr_job_id: "job-fired-1" } };
    const out = await cancelScheduledHandler({ workspaceId: "ws-1", contentItemId: "item-1" }, { sdrBaseUrl: sdr.baseUrl, token: "ws-key", db: makeMockContentDb([fired]) });
    expect(out.status).toBe(400);
  });
});
