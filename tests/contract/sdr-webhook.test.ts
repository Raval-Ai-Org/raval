...
function seed() {
  return makeMockDb({
    ...
    content_publications: [{
      ...
      platform: "twitter", account_id: "tw-1", status: "publishing",
    }],
    content_items: [{ id: "item-1", workspace_id: "ws-1", body: "x", media_url: null, status: "publishing", meta: { platform: "twitter" } }],
  });
}

const payload = (status: string, extra: Record<string, unknown> = {}) =>
  JSON.stringify({ event: `post.${status}`, data: { post_id: "job-1", target_id: "target-1", status, ...extra } });
...
describe("handleSdrWebhook", () => {
  ...
  it("rejects an invalid signature with 401 and changes NO state (FR-021/SC-009)", async () => {
    ...
    const out = await handleSdrWebhook({ rawBody: body, signature: "sha256=deadbeef", eventType: "post.published" }, { db });
  ...
  it("returns 404 for an unknown delivery (no state change)", async () => {
    const db = makeMockDb({ workspace_sdr: [{ workspace_id: "ws-1", webhook_secret: encryptSecret(SECRET) }] });
    const body = payload("published");
    const out = await handleSdrWebhook({ rawBody: body, signature: sign(body), eventType: "post.published" }, { db });
  ...
  it("rejects an oversized body with 413 before verification (C1)", async () => {
    ...
    const out = await handleSdrWebhook({ rawBody: big, signature: sign(big), eventType: "post.published", maxBodyBytes: 1000 }, { db });
