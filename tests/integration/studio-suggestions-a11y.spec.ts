...
async function readCards(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    ...
    return tries.map((tryBtn) => {
      ...
      const dismissBtn =
        card?.querySelector<HTMLButtonElement>('button[aria-label="Dismiss suggestion"]') ?? null;
      const labelSpan =
        bodyBtn?.querySelector<HTMLSpanElement>("span[title]:not([title=''])") ?? null;
...
test.describe("Studio suggestion a11y", () => {
  test.beforeEach(async ({ context, page }) => {
    ...
    await expect(page.getByRole("button", { name: /^Run suggestion:/ }).first()).toBeVisible({
      timeout: 15_000,
    });
  });
  ...
  test("every card exposes accessible names + native tooltip titles", async ({ page }) => {
    ...
    for (const c of cards) {
      ...
      expect(c.bodyTitle, `body title for ${c.label}`).toMatch(
        new RegExp(`^${c.label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} — .+`),
      );
  ...
  test("tab order per card: body → Try → Dismiss, in DOM order", async ({ page }) => {
    ...
    const classify = () =>
      page.evaluate(() => {
        ...
        if (aria.startsWith("Run suggestion: "))
          return `try:${aria.slice("Run suggestion: ".length)}`;
  ...
  test("Dismiss is keyboard-activatable and removes the card", async ({ page }) => {
    ...
    await page.keyboard.press("Enter");
    await expect(page.getByRole("button", { name: `Run suggestion: ${target.label}` })).toHaveCount(
      0,
      { timeout: 3_000 },
    );
  });
