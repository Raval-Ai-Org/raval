...
const SUGGESTIONS = [
  ...
  },
  {
    ...
    hint: "0 published in 7 days",
    accent: "green",
    icon: "Calendar",
    intent: "plan-week",
    prompt: "Plan this week's content.",
  },
];
    ...
    const clickAndExpect = async (labelRe: RegExp, expectedEvent: string) => {
      ...
      await expect
        .poll(
          async () =>
            page.evaluate(
              (k) => (window as unknown as { __ev: string[] }).__ev.includes(k),
              expectedEvent,
            ),
          { timeout: 3_000 },
        )
