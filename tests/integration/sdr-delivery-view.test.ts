...
describe("delivery view data (webhook → per-platform status + live link)", () => {
  it("publishes a live link + status per platform that the Studio renders", async () => {
    ...
    await handleSdrWebhook(
      { rawBody: li, signature: sign(li), eventType: "post.published" },
      { db },
    );
    ...
    await handleSdrWebhook(
      { rawBody: tw, signature: sign(tw), eventType: "post.retrying" },
      { db },
    );

    const rows = db._state.content_publications;
    expect(rows.find((r) => r.sdr_target_id === "t-li")).toMatchObject({
      status: "published",
      platform_post_url: "https://linkedin.com/posts/1",
    });
  ...
  it("a partial failure surfaces as partial_failed with the failure reason", async () => {
    ...
    const a = JSON.stringify({
      ...
      data: {
        ...
        target_id: "t-a",
        status: "published",
        platform_post_url: "https://linkedin.com/posts/9",
      },
    });
    ...
    expect(db._state.content_publications.find((r) => r.sdr_target_id === "t-b").last_error).toBe(
      "Duplicate content",
    );
  });
});
